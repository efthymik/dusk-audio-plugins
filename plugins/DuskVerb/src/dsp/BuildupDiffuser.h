...
public:
    void prepare (double sampleRate, int /*maxBlockSize*/)
    {
        ...
        const float r = static_cast<float> (sampleRate / 44100.0);   // delays are 44.1k-referenced
        for (int s = 0; s < kNumStages; ++s)
        {
            ...
            apL_[s].prepare (bufL, static_cast<float> (dL), kLfoRate[s], 1.5f, 0.13f * s,      sampleRate);
            apR_[s].prepare (bufR, static_cast<float> (dR), kLfoRate[s], 1.5f, 0.13f * s + 0.5f, sampleRate);
...
private:
    ...
    // Increasing, mutually-prime delays (samples @ 44.1k) → ~84 ms cumulative smear
    // with rising echo density (the buildup). Tuned so the DenseHall tail's energy
    // centroid (t50) lands ~204 ms on Bright Hall (a longer cascade overshot to
    // ~294 ms). L/R differ for decorrelation.
