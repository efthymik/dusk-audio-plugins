...
// any other preset. The class has been renamed to LiveRoomPresetEngine to avoid
// ODR conflicts.

#include "LiveRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetLiveRoom in AlgorithmConfig.h
...
class LiveRoomPresetEngine
{
public:
    LiveRoomPresetEngine();
...
void LiveRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LiveRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LiveRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void LiveRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LiveRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
LiveRoomPresetEngine::LiveRoomPresetEngine()
{
...
void LiveRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void LiveRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void LiveRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float LiveRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void LiveRoomPresetEngine::setDecayTime (float seconds)
{
...
void LiveRoomPresetEngine::setBassMultiply (float mult)
{
...
void LiveRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void LiveRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void LiveRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void LiveRoomPresetEngine::setAirDampingScale (float scale)
{
...
void LiveRoomPresetEngine::setModDepth (float depth)
{
...
void LiveRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void LiveRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void LiveRoomPresetEngine::setSize (float size)
{
...
void LiveRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void LiveRoomPresetEngine::setDelayScale (float scale)
{
...
void LiveRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void LiveRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void LiveRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void LiveRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void LiveRoomPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void LiveRoomPresetEngine::setSizeRange (float min, float max)
{
...
void LiveRoomPresetEngine::setDecayBoost (float boost)
{
...
void LiveRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LiveRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LiveRoomPresetEngine::clearBuffers()
{
...
void LiveRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LiveRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void LiveRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LiveRoomPresetWrapper : public PresetEngineBase
{
public:
    LiveRoomPresetWrapper() = default;
    ~LiveRoomPresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new LiveRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLiveRoomPresetRegistrar ("PresetLiveRoom", &createLiveRoomPreset);
