...
// any other preset. The class has been renamed to StudioRoomPresetEngine to avoid
// ODR conflicts.

#include "StudioRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetStudioRoom in AlgorithmConfig.h
...
class StudioRoomPresetEngine
{
public:
    StudioRoomPresetEngine();
...
void StudioRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void StudioRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float StudioRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void StudioRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void StudioRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
StudioRoomPresetEngine::StudioRoomPresetEngine()
{
...
void StudioRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void StudioRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void StudioRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float StudioRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void StudioRoomPresetEngine::setDecayTime (float seconds)
{
...
void StudioRoomPresetEngine::setBassMultiply (float mult)
{
...
void StudioRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void StudioRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void StudioRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void StudioRoomPresetEngine::setAirDampingScale (float scale)
{
...
void StudioRoomPresetEngine::setModDepth (float depth)
{
...
void StudioRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void StudioRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void StudioRoomPresetEngine::setSize (float size)
{
...
void StudioRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void StudioRoomPresetEngine::setDelayScale (float scale)
{
...
void StudioRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void StudioRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void StudioRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void StudioRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void StudioRoomPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void StudioRoomPresetEngine::setSizeRange (float min, float max)
{
...
void StudioRoomPresetEngine::setDecayBoost (float boost)
{
...
void StudioRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void StudioRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void StudioRoomPresetEngine::clearBuffers()
{
...
void StudioRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void StudioRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void StudioRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class StudioRoomPresetWrapper : public PresetEngineBase
{
public:
    StudioRoomPresetWrapper() = default;
    ~StudioRoomPresetWrapper() override = default;
...
private:
    StudioRoomPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new StudioRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gStudioRoomPresetRegistrar ("PresetStudioRoom", &createStudioRoomPreset);
