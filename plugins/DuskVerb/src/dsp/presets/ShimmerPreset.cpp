...
// any other preset. The class has been renamed to ShimmerPresetEngine to avoid
// ODR conflicts.

#include "ShimmerPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetShimmer in AlgorithmConfig.h
    ...
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.411793f — size range now absolute
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.353889f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -12.0f, -12.0f, -12.0f, -12.0f, -12.0f, -12.0f, -12.0f, -12.0f, 2.4f, -18.0f, -18.0f, 24.0f };
...
void ShimmerPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void ShimmerPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float ShimmerPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void ShimmerPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void ShimmerPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
ShimmerPresetEngine::ShimmerPresetEngine()
{
...
void ShimmerPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void ShimmerPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void ShimmerPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float ShimmerPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void ShimmerPresetEngine::setDecayTime (float seconds)
{
...
void ShimmerPresetEngine::setBassMultiply (float mult)
{
...
void ShimmerPresetEngine::setTrebleMultiply (float mult)
{
...
void ShimmerPresetEngine::setCrossoverFreq (float hz)
{
...
void ShimmerPresetEngine::setHighCrossoverFreq (float hz)
{
...
void ShimmerPresetEngine::setAirDampingScale (float scale)
{
...
void ShimmerPresetEngine::setModDepth (float depth)
{
...
void ShimmerPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void ShimmerPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void ShimmerPresetEngine::setSize (float size)
{
...
void ShimmerPresetEngine::setSoftOnsetMs (float ms)
{
...
void ShimmerPresetEngine::setDelayScale (float scale)
{
...
void ShimmerPresetEngine::setNoiseModDepth (float samples)
{
...
void ShimmerPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void ShimmerPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void ShimmerPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void ShimmerPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void ShimmerPresetEngine::setSizeRange (float min, float max)
{
...
void ShimmerPresetEngine::setDecayBoost (float boost)
{
...
void ShimmerPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void ShimmerPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void ShimmerPresetEngine::clearBuffers()
{
...
void ShimmerPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void ShimmerPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void ShimmerPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class ShimmerPresetWrapper : public PresetEngineBase
{
public:
    ShimmerPresetWrapper() = default;
    ~ShimmerPresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new ShimmerPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gShimmerPresetRegistrar ("PresetShimmer", &createShimmerPreset);
