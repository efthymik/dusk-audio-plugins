...
// any other preset. The class has been renamed to DrumRoomPresetEngine to avoid
// ODR conflicts.

#include "DrumRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetDrumRoom in AlgorithmConfig.h
...
class DrumRoomPresetEngine
{
public:
    DrumRoomPresetEngine();
...
void DrumRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void DrumRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float DrumRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void DrumRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void DrumRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
DrumRoomPresetEngine::DrumRoomPresetEngine()
{
...
void DrumRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void DrumRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void DrumRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float DrumRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void DrumRoomPresetEngine::setDecayTime (float seconds)
{
...
void DrumRoomPresetEngine::setBassMultiply (float mult)
{
...
void DrumRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void DrumRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void DrumRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void DrumRoomPresetEngine::setAirDampingScale (float scale)
{
...
void DrumRoomPresetEngine::setModDepth (float depth)
{
...
void DrumRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void DrumRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void DrumRoomPresetEngine::setSize (float size)
{
...
void DrumRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void DrumRoomPresetEngine::setDelayScale (float scale)
{
...
void DrumRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void DrumRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void DrumRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void DrumRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void DrumRoomPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void DrumRoomPresetEngine::setSizeRange (float min, float max)
{
...
void DrumRoomPresetEngine::setDecayBoost (float boost)
{
...
void DrumRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void DrumRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void DrumRoomPresetEngine::clearBuffers()
{
...
void DrumRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void DrumRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void DrumRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class DrumRoomPresetWrapper : public PresetEngineBase
{
public:
    DrumRoomPresetWrapper() = default;
    ~DrumRoomPresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new DrumRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gDrumRoomPresetRegistrar ("PresetDrumRoom", &createDrumRoomPreset);
