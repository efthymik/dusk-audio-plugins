...
//
// Per-preset reverb engine for "Ambience Plate".
...
// any other preset. The class has been renamed to AmbiencePlatePresetEngine to avoid
// ODR conflicts.

#include "AmbiencePlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetAmbiencePlate in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.356741f;
    constexpr float kBakedSizeRangeMax       = 1.070223f;
    constexpr float kBakedAirDampingScale    = 0.8f;
    constexpr float kBakedNoiseModDepth      = 8.0f;
    constexpr float kBakedTrebleMultScale    = 0.75f;
    constexpr float kBakedTrebleMultScaleMax = 1.5f;
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 1.11f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.935914f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.713482f — size range now absolute
    constexpr float kVvTiltLowDb         = 0.472775f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -0.381782f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.920965f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.813794f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 3.86618f, 4.32772f, -0.601169f, 0.790713f, 4.75813f, 1.38391f, 4.20049f, 5.48218f, 3.88185f, 5.82f, 6.29f, 24.0f };
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9419f,
        0.7197f, 0.7051f, 0.6078f, 0.6250f, 0.7078f, 0.6517f, 0.6404f, 0.7105f,
        0.7832f, 0.8006f, 0.8374f, 0.9574f, 1.0000f, 0.7675f, 0.6849f, 0.6015f,
        0.5926f, 0.5644f, 0.5658f, 0.6086f, 0.6147f, 0.7137f, 0.8958f, 1.0000f,
        1.0000f, 1.0000f, 1.0000f, 0.8429f, 0.6295f, 0.7316f, 0.8825f, 0.7878f,
        0.8630f, 0.8676f, 1.0000f, 0.9904f, 1.0000f, 1.0000f, 0.9897f, 0.8301f,
        0.7582f, 0.7105f, 0.8200f, 1.0000f, 1.0000f, 1.0000f, 0.8817f, 0.7622f,
        0.6394f, 0.6179f, 0.6618f, 0.8607f, 0.8237f, 0.9137f, 0.9341f, 1.0000f
    };
...
class AmbiencePlatePresetEngine
{
public:
    AmbiencePlatePresetEngine();
...
private:
    ...
    struct Tank
    {
        ...
        // Cached jitter values for freeze (avoid snap when frozen)
...
void AmbiencePlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void AmbiencePlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float AmbiencePlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void AmbiencePlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void AmbiencePlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
AmbiencePlatePresetEngine::AmbiencePlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void AmbiencePlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void AmbiencePlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        for (int t = 0; t < kNumTanks; ++t)
        {
            ...
            float jitter1 = frozen_ ? tank.lastJitter1 : nextDrift (tank.noiseState) * effectiveNoiseMod;
            ...
            float jitter2 = frozen_ ? tank.lastJitter2 : nextDrift (tank.noiseState) * effectiveNoiseMod;
...
// -----------------------------------------------------------------------
float AmbiencePlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void AmbiencePlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void AmbiencePlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void AmbiencePlatePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void AmbiencePlatePresetEngine::setCrossoverFreq (float hz)
{
...
void AmbiencePlatePresetEngine::setModDepth (float depth)
{
...
void AmbiencePlatePresetEngine::setNoiseModDepth (float samples)
{
...
void AmbiencePlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void AmbiencePlatePresetEngine::setSize (float size)
{
...
void AmbiencePlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void AmbiencePlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void AmbiencePlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void AmbiencePlatePresetEngine::setAirDampingScale (float scale)
{
...
void AmbiencePlatePresetEngine::setSizeRange (float min, float max)
{
...
void AmbiencePlatePresetEngine::setDecayBoost (float boost)
{
...
void AmbiencePlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void AmbiencePlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void AmbiencePlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void AmbiencePlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void AmbiencePlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void AmbiencePlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class AmbiencePlatePresetWrapper : public PresetEngineBase
{
public:
    AmbiencePlatePresetWrapper() = default;
    ~AmbiencePlatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Ambience Plate"; }
...
private:
    AmbiencePlatePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new AmbiencePlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gAmbiencePlatePresetRegistrar ("PresetAmbiencePlate", &createAmbiencePlatePreset);
