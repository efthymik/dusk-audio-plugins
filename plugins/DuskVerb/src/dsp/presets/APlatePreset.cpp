...
//
// Per-preset reverb engine for "A Plate".
...
// any other preset. The class has been renamed to APlatePresetEngine to avoid
// ODR conflicts.

#include "APlatePreset.h"
...
namespace {
    ...
    constexpr float kVvStereoWidth       = 0.930918f;
    ...
    // Max correction magnitude for this preset: 11.63 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 1.0000f, 1.0000f, 0.8980f, 0.4416f, 0.3190f, 0.2687f, 0.2706f,
        0.2609f, 0.2965f, 0.3022f, 0.3865f, 0.3116f, 0.2321f, 0.1848f, 0.2014f,
        0.3002f, 0.3524f, 0.3367f, 0.2936f, 0.2733f, 0.3180f, 0.4081f, 0.5953f,
        0.6248f, 0.4951f, 0.3543f, 0.3143f, 0.3027f, 0.3858f, 0.4328f, 0.4575f,
        0.3845f, 0.3817f, 0.5675f, 0.6411f, 0.6562f, 0.4338f, 0.4120f, 0.3621f,
        0.3869f, 0.5310f, 0.5547f, 0.4636f, 0.4616f, 0.5646f, 0.7457f, 0.7705f,
        0.6459f, 0.5760f, 0.5922f, 0.8397f, 0.9250f, 0.7197f, 0.6609f, 1.0000f
    };
...
void APlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void APlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float APlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void APlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void APlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
APlatePresetEngine::APlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void APlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void APlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float APlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void APlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void APlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void APlatePresetEngine::setTrebleMultiply (float mult)
{
...
void APlatePresetEngine::setModDepth (float depth)
{
...
void APlatePresetEngine::setNoiseModDepth (float samples)
{
...
void APlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void APlatePresetEngine::setSize (float size)
{
...
void APlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void APlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void APlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void APlatePresetEngine::setAirDampingScale (float scale)
{
...
void APlatePresetEngine::setDecayBoost (float boost)
{
...
void APlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void APlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void APlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void APlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void APlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void APlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class APlatePresetWrapper : public PresetEngineBase
{
public:
    APlatePresetWrapper() = default;
    ~APlatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "A Plate"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new APlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gAPlatePresetRegistrar ("PresetAPlate", &createAPlatePreset);
