// Per-preset reverb engine for "Dark Chamber".
...
// any other preset. The class has been renamed to DarkChamberPresetEngine to avoid
// ODR conflicts.

#include "DarkChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetDarkChamber in AlgorithmConfig.h
...
class DarkChamberPresetEngine
{
public:
    DarkChamberPresetEngine();
...
void DarkChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void DarkChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float DarkChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void DarkChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void DarkChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
DarkChamberPresetEngine::DarkChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void DarkChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void DarkChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float DarkChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void DarkChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void DarkChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void DarkChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void DarkChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void DarkChamberPresetEngine::setModDepth (float depth)
{
...
void DarkChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void DarkChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void DarkChamberPresetEngine::setSize (float size)
{
...
void DarkChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void DarkChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void DarkChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void DarkChamberPresetEngine::setAirDampingScale (float scale)
{
...
void DarkChamberPresetEngine::setSizeRange (float min, float max)
{
...
void DarkChamberPresetEngine::setDecayBoost (float boost)
{
...
void DarkChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void DarkChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void DarkChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void DarkChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void DarkChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void DarkChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class DarkChamberPresetWrapper : public PresetEngineBase
{
public:
    DarkChamberPresetWrapper() = default;
    ~DarkChamberPresetWrapper() override = default;
...
private:
    DarkChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new DarkChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gDarkChamberPresetRegistrar ("PresetDarkChamber", &createDarkChamberPreset);
