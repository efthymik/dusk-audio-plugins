...
//
// Per-preset reverb engine for "Large Plate".
...
namespace {
    ...
    constexpr float kBakedSizeRangeMin       = 0.781905f;
    constexpr float kBakedSizeRangeMax       = 2.345715f;
    ...
    constexpr float kVvAirDampingScale   = 0.755421f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 1.56381f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.11599f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -1.80672f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.884931f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.662458f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -2.06449f, -0.365793f, -4.56588f, -0.0700238f, -1.71754f, -2.81352f, -0.805786f, -1.77201f, 0.279273f, -3.83799f, -7.80524f, 12.0f };
...
void RichPlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }
...
public:
    ...
    const char* getPresetName() const override { return "Large Plate"; }
