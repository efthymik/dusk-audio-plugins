...
// any other preset. The class has been renamed to RichPlatePresetEngine to avoid
// ODR conflicts.

#include "RichPlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetRichPlate in AlgorithmConfig.h
    ...
    // -----------------------------------------------------------------
    // Structural constants — Rich Plate (long, lush, balanced).
    // Target: EMT 140 "medium" damper, variant 4 → RT60 ~6.1s, centroid ~4.3 kHz.
    // Same length as Dark Plate but slightly darker centroid; lush modulation.
    // -----------------------------------------------------------------
    constexpr float kVvCrossoverHz       = 1000.0f;
    constexpr float kVvHighCrossoverHz   = 3000.0f;   // air split low — Rich's character is strong HF damping
    constexpr float kVvAirDampingScale   = 0.55f;     // very aggressive HF damping
    constexpr float kVvDelayScale        = 1.0f;
    constexpr float kVvTiltLowDb         = 0.0f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -18.0f;    // strong HF cut to land 4.3 kHz centroid
    constexpr float kVvTiltHighHz        = 3000.0f;
    constexpr float kVvStereoWidth       = 0.95f;     // wider stereo for lush ballad/pad use
...
class RichPlatePresetEngine
{
public:
    RichPlatePresetEngine();
...
void RichPlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void RichPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float RichPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void RichPlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void RichPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
RichPlatePresetEngine::RichPlatePresetEngine()
{
...
void RichPlatePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void RichPlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void RichPlatePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float RichPlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void RichPlatePresetEngine::setDecayTime (float seconds)
{
...
void RichPlatePresetEngine::setBassMultiply (float mult)
{
...
void RichPlatePresetEngine::setTrebleMultiply (float mult)
{
...
void RichPlatePresetEngine::setCrossoverFreq (float hz)
{
...
void RichPlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void RichPlatePresetEngine::setAirDampingScale (float scale)
{
...
void RichPlatePresetEngine::setModDepth (float depth)
{
...
void RichPlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void RichPlatePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void RichPlatePresetEngine::setSize (float size)
{
...
void RichPlatePresetEngine::setSoftOnsetMs (float ms)
{
...
void RichPlatePresetEngine::setDelayScale (float scale)
{
...
void RichPlatePresetEngine::setNoiseModDepth (float samples)
{
...
void RichPlatePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void RichPlatePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void RichPlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void RichPlatePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void RichPlatePresetEngine::setSizeRange (float min, float max)
{
...
void RichPlatePresetEngine::setDecayBoost (float boost)
{
...
void RichPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void RichPlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void RichPlatePresetEngine::clearBuffers()
{
...
void RichPlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void RichPlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void RichPlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class RichPlatePresetWrapper : public PresetEngineBase
{
public:
    RichPlatePresetWrapper() = default;
    ~RichPlatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Rich Plate"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new RichPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gRichPlatePresetRegistrar ("PresetRichPlate", &createRichPlatePreset);
