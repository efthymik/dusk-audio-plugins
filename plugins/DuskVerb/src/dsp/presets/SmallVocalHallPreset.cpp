...
//
// Per-preset reverb engine for "Small Vocal Hall".
...
// any other preset. The class has been renamed to SmallVocalHallPresetEngine to avoid
// ODR conflicts.

#include "SmallVocalHallPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetSmallVocalHall in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.59388f;
    constexpr float kBakedSizeRangeMax       = 1.78164f;
    constexpr float kBakedAirDampingScale    = 0.8f;
    constexpr float kBakedNoiseModDepth      = 8.0f;
    constexpr float kBakedTrebleMultScale    = 0.82f;
    constexpr float kBakedTrebleMultScaleMax = 1.5f;
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 1.23f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.759335f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 1.18776f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.16051f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -5.70383f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.830561f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.159298f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 12.0f, -3.03369f, 2.72254f, 2.73257f, 1.02088f, -0.665043f, 1.93962f, 2.58949f, -0.757323f, 3.11589f, 12.0f, 12.0f };
    ...
    // -----------------------------------------------------------------
    // Band 3-4 damped to shorten HF tail — freqDecay_8k was 3.28 (DV 8k
    // accumulating too much relative to VV's decay). More damping brings it
    // into spec.
    constexpr float kFiveBandMult[5] = { 1.00f, 1.00f, 1.00f, 0.50f, 0.40f };
...
class SmallVocalHallPresetEngine
{
public:
    SmallVocalHallPresetEngine();
...
void SmallVocalHallPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void SmallVocalHallPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float SmallVocalHallPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void SmallVocalHallPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void SmallVocalHallPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
SmallVocalHallPresetEngine::SmallVocalHallPresetEngine()
{
...
// -----------------------------------------------------------------------
void SmallVocalHallPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float SmallVocalHallPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void SmallVocalHallPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SmallVocalHallPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SmallVocalHallPresetEngine::setTrebleMultiply (float mult)
{
...
void SmallVocalHallPresetEngine::setSize (float size)
{
...
void SmallVocalHallPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void SmallVocalHallPresetEngine::setAirDampingScale (float scale)
{
...
void SmallVocalHallPresetEngine::setSizeRange (float min, float max)
{
...
void SmallVocalHallPresetEngine::setDecayBoost (float boost)
{
...
void SmallVocalHallPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
// -----------------------------------------------------------------------
void SmallVocalHallPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void SmallVocalHallPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void SmallVocalHallPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class SmallVocalHallPresetWrapper : public PresetEngineBase
{
public:
    SmallVocalHallPresetWrapper() = default;
    ~SmallVocalHallPresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new SmallVocalHallPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gSmallVocalHallPresetRegistrar ("PresetSmallVocalHall", &createSmallVocalHallPreset);
