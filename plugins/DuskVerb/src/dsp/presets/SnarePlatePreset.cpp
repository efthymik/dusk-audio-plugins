...
// any other preset. The class has been renamed to SnarePlatePresetEngine to avoid
// ODR conflicts.

#include "SnarePlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetSnarePlate in AlgorithmConfig.h
...
class SnarePlatePresetEngine
{
public:
    SnarePlatePresetEngine();
...
void SnarePlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void SnarePlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float SnarePlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void SnarePlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void SnarePlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
SnarePlatePresetEngine::SnarePlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void SnarePlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void SnarePlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float SnarePlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void SnarePlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SnarePlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SnarePlatePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SnarePlatePresetEngine::setCrossoverFreq (float hz)
{
...
void SnarePlatePresetEngine::setModDepth (float depth)
{
...
void SnarePlatePresetEngine::setNoiseModDepth (float samples)
{
...
void SnarePlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void SnarePlatePresetEngine::setSize (float size)
{
...
void SnarePlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void SnarePlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void SnarePlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void SnarePlatePresetEngine::setAirDampingScale (float scale)
{
...
void SnarePlatePresetEngine::setSizeRange (float min, float max)
{
...
void SnarePlatePresetEngine::setDecayBoost (float boost)
{
...
void SnarePlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void SnarePlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void SnarePlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void SnarePlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void SnarePlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void SnarePlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class SnarePlatePresetWrapper : public PresetEngineBase
{
public:
    SnarePlatePresetWrapper() = default;
    ~SnarePlatePresetWrapper() override = default;
...
private:
    SnarePlatePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new SnarePlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gSnarePlatePresetRegistrar ("PresetSnarePlate", &createSnarePlatePreset);
