...
//
// Per-preset reverb engine for "Big Ambience Gate".
...
// any other preset. The class has been renamed to BigAmbienceGatePresetEngine to avoid
// ODR conflicts.

#include "BigAmbienceGatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetBigAmbienceGate in AlgorithmConfig.h
    ...
    constexpr float kVvAirDampingScale   = 1.00405f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.4f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.5064f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -4.37683f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.873313f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.261f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 4.36899f, -12.0f, 5.65727f, -7.31118f, 1.22135f, -3.21407f, -1.75152f, -1.29157f, -1.80015f, -2.56411f, 0.358565f, 20.0f };
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 0.9933f, 0.9331f, 0.8220f, 0.7695f, 0.6276f, 0.4714f, 0.4998f,
        0.6190f, 0.7705f, 0.7865f, 0.8417f, 0.9959f, 1.0000f, 1.0000f, 0.9956f,
        0.7282f, 0.5046f, 0.4831f, 0.6593f, 0.8587f, 1.0000f, 1.0000f, 1.0000f,
        1.0000f, 0.9565f, 0.8687f, 0.8608f, 1.0000f, 1.0000f, 1.0000f, 1.0000f,
        0.9848f, 0.9842f, 0.8933f, 0.8305f, 0.7761f, 1.0000f, 1.0000f, 1.0000f,
        0.8727f, 1.0000f, 1.0000f, 1.0000f, 0.9840f, 0.7298f, 0.6069f, 0.5389f,
        0.6386f, 0.6285f, 0.8362f, 0.9284f, 0.9707f, 0.9304f, 0.8824f, 1.0000f
    };
...
class BigAmbienceGatePresetEngine
{
public:
    BigAmbienceGatePresetEngine();
...
private:
    ...
    float limiterReleaseMs_ = 50.0f;      // Stored release time for recomputation on SR change
...
void BigAmbienceGatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void BigAmbienceGatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float BigAmbienceGatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void BigAmbienceGatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void BigAmbienceGatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
BigAmbienceGatePresetEngine::BigAmbienceGatePresetEngine()
{
...
void BigAmbienceGatePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void BigAmbienceGatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void BigAmbienceGatePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float BigAmbienceGatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void BigAmbienceGatePresetEngine::setDecayTime (float seconds)
{
...
void BigAmbienceGatePresetEngine::setBassMultiply (float mult)
{
...
void BigAmbienceGatePresetEngine::setTrebleMultiply (float mult)
{
...
void BigAmbienceGatePresetEngine::setCrossoverFreq (float hz)
{
...
void BigAmbienceGatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void BigAmbienceGatePresetEngine::setAirDampingScale (float scale)
{
...
void BigAmbienceGatePresetEngine::setModDepth (float depth)
{
...
void BigAmbienceGatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void BigAmbienceGatePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void BigAmbienceGatePresetEngine::setSize (float size)
{
...
void BigAmbienceGatePresetEngine::setSoftOnsetMs (float ms)
{
...
void BigAmbienceGatePresetEngine::setDelayScale (float scale)
{
...
void BigAmbienceGatePresetEngine::setNoiseModDepth (float samples)
{
...
void BigAmbienceGatePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void BigAmbienceGatePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void BigAmbienceGatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void BigAmbienceGatePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void BigAmbienceGatePresetEngine::setSizeRange (float min, float max)
{
...
void BigAmbienceGatePresetEngine::setDecayBoost (float boost)
{
...
void BigAmbienceGatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void BigAmbienceGatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void BigAmbienceGatePresetEngine::clearBuffers()
{
...
void BigAmbienceGatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void BigAmbienceGatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void BigAmbienceGatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class BigAmbienceGatePresetWrapper : public PresetEngineBase
{
public:
    BigAmbienceGatePresetWrapper() = default;
    ~BigAmbienceGatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Big Ambience Gate"; }
...
private:
    BigAmbienceGatePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new BigAmbienceGatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gBigAmbienceGatePresetRegistrar ("PresetBigAmbienceGate", &createBigAmbienceGatePreset);
