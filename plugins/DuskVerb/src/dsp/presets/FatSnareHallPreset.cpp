...
//
// Per-preset reverb engine for "Fat Snare Hall".
...
// any other preset. The class has been renamed to FatSnareHallPresetEngine to avoid
// ODR conflicts.

#include "FatSnareHallPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetFatSnareHall in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.572285f;
    constexpr float kBakedSizeRangeMax       = 1.716855f;
    ...
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 1.14457f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.28788f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -6.0f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.8559f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 1.08529f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 12.0f, -12.0f, 12.0f, -3.57416f, -4.31273f, -9.1794f, 8.43158f, -12.0f, -12.0f, -10.8855f, 12.0f, 12.0f };
...
class FatSnareHallPresetEngine
{
public:
    FatSnareHallPresetEngine();
...
void FatSnareHallPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void FatSnareHallPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float FatSnareHallPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void FatSnareHallPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void FatSnareHallPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
FatSnareHallPresetEngine::FatSnareHallPresetEngine()
{
...
// -----------------------------------------------------------------------
void FatSnareHallPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void FatSnareHallPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float FatSnareHallPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void FatSnareHallPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatSnareHallPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatSnareHallPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatSnareHallPresetEngine::setCrossoverFreq (float hz)
{
...
void FatSnareHallPresetEngine::setModDepth (float depth)
{
...
void FatSnareHallPresetEngine::setNoiseModDepth (float samples)
{
...
void FatSnareHallPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void FatSnareHallPresetEngine::setSize (float size)
{
...
void FatSnareHallPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void FatSnareHallPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void FatSnareHallPresetEngine::setHighCrossoverFreq (float hz)
{
...
void FatSnareHallPresetEngine::setAirDampingScale (float scale)
{
...
void FatSnareHallPresetEngine::setSizeRange (float min, float max)
{
...
void FatSnareHallPresetEngine::setDecayBoost (float boost)
{
...
void FatSnareHallPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void FatSnareHallPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void FatSnareHallPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void FatSnareHallPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void FatSnareHallPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void FatSnareHallPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class FatSnareHallPresetWrapper : public PresetEngineBase
{
public:
    FatSnareHallPresetWrapper() = default;
    ~FatSnareHallPresetWrapper() override = default;
    ...
    void prepare (double sampleRate, int maxBlockSize) override
    {
        ...
        // This ensures engine_.prepare() allocates buffers with the correct range.
    ...
    const char* getPresetName() const override { return "Fat Snare Hall"; }
...
private:
    FatSnareHallPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new FatSnareHallPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gFatSnareHallPresetRegistrar ("PresetFatSnareHall", &createFatSnareHallPreset);
