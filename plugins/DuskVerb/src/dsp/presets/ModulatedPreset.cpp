...
// any other preset. The class has been renamed to ModulatedPresetEngine to avoid
// ODR conflicts.

#include "ModulatedPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetModulated in AlgorithmConfig.h
...
class ModulatedPresetEngine
{
public:
    ModulatedPresetEngine();
...
void ModulatedPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void ModulatedPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float ModulatedPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void ModulatedPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void ModulatedPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
ModulatedPresetEngine::ModulatedPresetEngine()
{
...
void ModulatedPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void ModulatedPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void ModulatedPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float ModulatedPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void ModulatedPresetEngine::setDecayTime (float seconds)
{
...
void ModulatedPresetEngine::setBassMultiply (float mult)
{
...
void ModulatedPresetEngine::setTrebleMultiply (float mult)
{
...
void ModulatedPresetEngine::setCrossoverFreq (float hz)
{
...
void ModulatedPresetEngine::setHighCrossoverFreq (float hz)
{
...
void ModulatedPresetEngine::setAirDampingScale (float scale)
{
...
void ModulatedPresetEngine::setModDepth (float depth)
{
...
void ModulatedPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void ModulatedPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void ModulatedPresetEngine::setSize (float size)
{
...
void ModulatedPresetEngine::setSoftOnsetMs (float ms)
{
...
void ModulatedPresetEngine::setDelayScale (float scale)
{
...
void ModulatedPresetEngine::setNoiseModDepth (float samples)
{
...
void ModulatedPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void ModulatedPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void ModulatedPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void ModulatedPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void ModulatedPresetEngine::setSizeRange (float min, float max)
{
...
void ModulatedPresetEngine::setDecayBoost (float boost)
{
...
void ModulatedPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void ModulatedPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void ModulatedPresetEngine::clearBuffers()
{
...
void ModulatedPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void ModulatedPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void ModulatedPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class ModulatedPresetWrapper : public PresetEngineBase
{
public:
    ModulatedPresetWrapper() = default;
    ~ModulatedPresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new ModulatedPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gModulatedPresetRegistrar ("PresetModulated", &createModulatedPreset);
