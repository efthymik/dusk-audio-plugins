...
// any other preset. The class has been renamed to LargePlatePresetEngine to avoid
// ODR conflicts.

#include "LargePlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetLargePlate in AlgorithmConfig.h
...
class LargePlatePresetEngine
{
public:
    LargePlatePresetEngine();
...
void LargePlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LargePlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LargePlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void LargePlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LargePlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
LargePlatePresetEngine::LargePlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void LargePlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void LargePlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float LargePlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void LargePlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargePlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargePlatePresetEngine::setTrebleMultiply (float mult)
{
...
void LargePlatePresetEngine::setModDepth (float depth)
{
...
void LargePlatePresetEngine::setNoiseModDepth (float samples)
{
...
void LargePlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void LargePlatePresetEngine::setSize (float size)
{
...
void LargePlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void LargePlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void LargePlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void LargePlatePresetEngine::setAirDampingScale (float scale)
{
...
void LargePlatePresetEngine::setDecayBoost (float boost)
{
...
void LargePlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LargePlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LargePlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void LargePlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LargePlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void LargePlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LargePlatePresetWrapper : public PresetEngineBase
{
public:
    LargePlatePresetWrapper() = default;
    ~LargePlatePresetWrapper() override = default;
...
private:
    LargePlatePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new LargePlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLargePlatePresetRegistrar ("PresetLargePlate", &createLargePlatePreset);
