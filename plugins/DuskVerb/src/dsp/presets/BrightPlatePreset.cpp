...
void BrightPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float BrightPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void BrightPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
void BrightPlatePresetEngine::setDecayBoost (float boost)
{
...
void BrightPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
class BrightPlatePresetWrapper : public PresetEngineBase
{
public:
    BrightPlatePresetWrapper() = default;
    ~BrightPlatePresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new BrightPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gBrightPlatePresetRegistrar ("PresetBrightPlate", &createBrightPlatePreset);
