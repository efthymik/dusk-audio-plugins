...
//
// Per-preset reverb engine for "Tight Ambience Gate".
...
// any other preset. The class has been renamed to TightAmbienceGatePresetEngine to avoid
// ODR conflicts.

#include "TightAmbienceGatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetTightAmbienceGate in AlgorithmConfig.h
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.69f — bass knob now shows real multiplier
    ...
    constexpr float kVvTiltLowDb         = -0.745057f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -4.59659f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.875472f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.139414f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -1.34036f, -0.13587f, -4.70806f, -0.241201f, -3.9101f, -3.15577f, -2.2695f, -2.39992f, -4.31118f, -5.9407f, 5.19714f, 22.0f };
...
class TightAmbienceGatePresetEngine
{
public:
    TightAmbienceGatePresetEngine();
...
void TightAmbienceGatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void TightAmbienceGatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float TightAmbienceGatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void TightAmbienceGatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void TightAmbienceGatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
TightAmbienceGatePresetEngine::TightAmbienceGatePresetEngine()
{
...
void TightAmbienceGatePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void TightAmbienceGatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void TightAmbienceGatePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float TightAmbienceGatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void TightAmbienceGatePresetEngine::setDecayTime (float seconds)
{
...
void TightAmbienceGatePresetEngine::setBassMultiply (float mult)
{
...
void TightAmbienceGatePresetEngine::setTrebleMultiply (float mult)
{
...
void TightAmbienceGatePresetEngine::setCrossoverFreq (float hz)
{
...
void TightAmbienceGatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void TightAmbienceGatePresetEngine::setAirDampingScale (float scale)
{
...
void TightAmbienceGatePresetEngine::setModDepth (float depth)
{
...
void TightAmbienceGatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void TightAmbienceGatePresetEngine::setSize (float size)
{
...
void TightAmbienceGatePresetEngine::setSoftOnsetMs (float ms)
{
...
void TightAmbienceGatePresetEngine::setDelayScale (float scale)
{
...
void TightAmbienceGatePresetEngine::setNoiseModDepth (float samples)
{
...
void TightAmbienceGatePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void TightAmbienceGatePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void TightAmbienceGatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void TightAmbienceGatePresetEngine::setDecayBoost (float boost)
{
...
void TightAmbienceGatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void TightAmbienceGatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void TightAmbienceGatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void TightAmbienceGatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void TightAmbienceGatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class TightAmbienceGatePresetWrapper : public PresetEngineBase
{
public:
    TightAmbienceGatePresetWrapper() = default;
    ~TightAmbienceGatePresetWrapper() override = default;
                  ...
                  float* outputL, float* outputR, int numSamples) override
    {
        ...
        // <<< PATCH_BODY_BEGIN:add_crest_softener >>>
        // Soft tanh saturation — reduces transient crest factor
        {
            constexpr float kKnee = 0.8466f;
            constexpr float kInvKnee = 1.0f / kKnee;
            for (int i = 0; i < numSamples; ++i)
            {
                outputL[i] = std::tanh (outputL[i] * kInvKnee) * kKnee;
                outputR[i] = std::tanh (outputR[i] * kInvKnee) * kKnee;
            }
        }
        // <<< PATCH_BODY_END:add_crest_softener >>>
    ...
    const char* getPresetName() const override { return "Tight Ambience Gate"; }
...
private:
    TightAmbienceGatePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new TightAmbienceGatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gTightAmbienceGatePresetRegistrar ("PresetTightAmbienceGate", &createTightAmbienceGatePreset);
