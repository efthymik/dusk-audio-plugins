...
// any other preset. The class has been renamed to DrumChamberPresetEngine to avoid
// ODR conflicts.

#include "DrumChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetDrumChamber in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.403768f;
    constexpr float kBakedSizeRangeMax       = 1.211305f;
    constexpr float kBakedAirDampingScale    = 0.8f;
    constexpr float kBakedNoiseModDepth      = 8.0f;
    constexpr float kBakedTrebleMultScale    = 0.98f;
    ...
    constexpr float kVvAirDampingScale   = 0.987798f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.807537f — size range now absolute
    constexpr float kVvTiltLowDb         = 0.794457f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -0.279958f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.8f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.905516f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 1.39555f, -1.04716f, 3.50437f, -2.12358f, 0.242056f, 2.85215f, 0.33061f, 3.31712f, 2.79735f, 1.24118f, 0.792197f, 1.99975f };
...
class DrumChamberPresetEngine
{
public:
    DrumChamberPresetEngine();
...
void DrumChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void DrumChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float DrumChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void DrumChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void DrumChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
DrumChamberPresetEngine::DrumChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void DrumChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void DrumChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float DrumChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void DrumChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void DrumChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void DrumChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void DrumChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void DrumChamberPresetEngine::setModDepth (float depth)
{
...
void DrumChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void DrumChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void DrumChamberPresetEngine::setSize (float size)
{
...
void DrumChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void DrumChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void DrumChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void DrumChamberPresetEngine::setAirDampingScale (float scale)
{
...
void DrumChamberPresetEngine::setSizeRange (float min, float max)
{
...
void DrumChamberPresetEngine::setDecayBoost (float boost)
{
...
void DrumChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void DrumChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void DrumChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void DrumChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void DrumChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void DrumChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class DrumChamberPresetWrapper : public PresetEngineBase
{
public:
    DrumChamberPresetWrapper() = default;
    ~DrumChamberPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Snare Plate"; }
...
private:
    DrumChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new DrumChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gDrumChamberPresetRegistrar ("PresetDrumChamber", &createDrumChamberPreset);
