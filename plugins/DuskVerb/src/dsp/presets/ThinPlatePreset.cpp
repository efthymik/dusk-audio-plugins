...
// any other preset. The class has been renamed to ThinPlatePresetEngine to avoid
// ODR conflicts.

#include "ThinPlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetThinPlate in AlgorithmConfig.h
    ...
    constexpr float kBakedBassMultScale      = 0.75f;
    ...
    // -----------------------------------------------------------------
    constexpr float kVvBassMultiply      = 0.795916f;
    constexpr float kVvTrebleMultiply    = 1.11095f;
    ...
    constexpr float kVvDelayScale        = 0.494763f;
    ...
    constexpr float kVvDecayTimeScale    = 0.965861f;
    ...
    // Max correction magnitude for this preset: 2.95 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 1.09056f, 0.217257f, 0.899442f, 0.191117f, 0.474557f, -0.368996f, 0.200829f, -0.344162f, -0.800631f, 0.00793135f, -0.132818f, -0.905373f };
...
class ThinPlatePresetEngine
{
public:
    ThinPlatePresetEngine();
...
void ThinPlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void ThinPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float ThinPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void ThinPlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void ThinPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
ThinPlatePresetEngine::ThinPlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void ThinPlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void ThinPlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float ThinPlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void ThinPlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void ThinPlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void ThinPlatePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void ThinPlatePresetEngine::setCrossoverFreq (float hz)
{
    crossoverFreq_ = std::max (hz, 20.0f);
    if (prepared_) updateDecayCoefficients();
}

void ThinPlatePresetEngine::setModDepth (float depth)
{
...
void ThinPlatePresetEngine::setNoiseModDepth (float samples)
{
...
void ThinPlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void ThinPlatePresetEngine::setSize (float size)
{
...
void ThinPlatePresetEngine::setFreeze (bool frozen) { frozen_ = frozen; }
...
void ThinPlatePresetEngine::setAirDampingScale (float scale)
{
...
void ThinPlatePresetEngine::setSizeRange (float min, float max)
{
...
void ThinPlatePresetEngine::setDecayBoost (float boost)
{
...
void ThinPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void ThinPlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void ThinPlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void ThinPlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void ThinPlatePresetEngine::updateDecayCoefficients()
{
...
void ThinPlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class ThinPlatePresetWrapper : public PresetEngineBase
{
public:
    ThinPlatePresetWrapper() = default;
    ~ThinPlatePresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new ThinPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gThinPlatePresetRegistrar ("PresetThinPlate", &createThinPlatePreset);
