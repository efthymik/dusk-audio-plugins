...
// any other preset. The class has been renamed to MedAmbiencePresetEngine to avoid
// ODR conflicts.

#include "MedAmbiencePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetMedAmbience in AlgorithmConfig.h
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 3.47393f, -3.03262f, 0.300123f, -1.03455f, -1.54747f, 0.384265f, -0.82722f, -0.483204f, -1.77527f, -6.36034f, 9.5303f, 12.0f };
...
class MedAmbiencePresetEngine
{
public:
    MedAmbiencePresetEngine();
...
void MedAmbiencePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void MedAmbiencePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float MedAmbiencePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void MedAmbiencePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void MedAmbiencePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
MedAmbiencePresetEngine::MedAmbiencePresetEngine()
{
...
// -----------------------------------------------------------------------
void MedAmbiencePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void MedAmbiencePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float MedAmbiencePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void MedAmbiencePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void MedAmbiencePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void MedAmbiencePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void MedAmbiencePresetEngine::setCrossoverFreq (float hz)
{
...
void MedAmbiencePresetEngine::setModDepth (float depth)
{
...
void MedAmbiencePresetEngine::setNoiseModDepth (float samples)
{
...
void MedAmbiencePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void MedAmbiencePresetEngine::setSize (float size)
{
...
void MedAmbiencePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void MedAmbiencePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void MedAmbiencePresetEngine::setHighCrossoverFreq (float hz)
{
...
void MedAmbiencePresetEngine::setAirDampingScale (float scale)
{
...
void MedAmbiencePresetEngine::setSizeRange (float min, float max)
{
...
void MedAmbiencePresetEngine::setDecayBoost (float boost)
{
...
void MedAmbiencePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void MedAmbiencePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void MedAmbiencePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void MedAmbiencePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void MedAmbiencePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void MedAmbiencePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class MedAmbiencePresetWrapper : public PresetEngineBase
{
public:
    MedAmbiencePresetWrapper() = default;
    ~MedAmbiencePresetWrapper() override = default;
...
private:
    MedAmbiencePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new MedAmbiencePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gMedAmbiencePresetRegistrar ("PresetMedAmbience", &createMedAmbiencePreset);
