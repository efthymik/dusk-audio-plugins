...
// any other preset. The class has been renamed to TightPlatePresetEngine to avoid
// ODR conflicts.

#include "TightPlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetTightPlate in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.440889f;
    constexpr float kBakedSizeRangeMax       = 1.322666f;
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 1.35f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.921594f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.881777f — size range now absolute
    constexpr float kVvTiltLowDb         = 0.807454f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -4.14046f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.876215f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 1.37502f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -0.374853f, 7.81429f, 2.86998f, -0.819993f, 1.52178f, 4.48408f, 0.642608f, 1.53301f, 2.92016f, -1.56676f, -1.28939f, 15.98f };
...
class TightPlatePresetEngine
{
public:
    TightPlatePresetEngine();
...
void TightPlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void TightPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float TightPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void TightPlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void TightPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
TightPlatePresetEngine::TightPlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void TightPlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void TightPlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float TightPlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void TightPlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void TightPlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void TightPlatePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void TightPlatePresetEngine::setCrossoverFreq (float hz)
{
...
void TightPlatePresetEngine::setModDepth (float depth)
{
...
void TightPlatePresetEngine::setNoiseModDepth (float samples)
{
...
void TightPlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void TightPlatePresetEngine::setSize (float size)
{
...
void TightPlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void TightPlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void TightPlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void TightPlatePresetEngine::setAirDampingScale (float scale)
{
...
void TightPlatePresetEngine::setSizeRange (float min, float max)
{
...
void TightPlatePresetEngine::setDecayBoost (float boost)
{
...
void TightPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void TightPlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void TightPlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void TightPlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void TightPlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void TightPlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class TightPlatePresetWrapper : public PresetEngineBase
{
public:
    TightPlatePresetWrapper() = default;
    ~TightPlatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Tight Plate"; }
...
private:
    TightPlatePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new TightPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gTightPlatePresetRegistrar ("PresetTightPlate", &createTightPlatePreset);
