...
// any other preset. The class has been renamed to FatPlatePresetEngine to avoid
// ODR conflicts.

#include "FatPlatePreset.h"
...
namespace {
    ...
    // -----------------------------------------------------------------
    constexpr float kVvBassMultiply      = 1.03627f;
    constexpr float kVvTrebleMultiply    = 1.00732f;
    ...
    constexpr float kVvDecayTimeScale    = 0.609995f;
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 1.12021f, -0.579501f, 3.23682f, 2.36529f, 0.989336f, -0.371281f, -0.533566f, -1.49768f, -4.43998f, -6.80663f, -8.23149f, -5.13806f };
...
class FatPlatePresetEngine
{
public:
    FatPlatePresetEngine();
...
void FatPlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void FatPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float FatPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void FatPlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void FatPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
FatPlatePresetEngine::FatPlatePresetEngine()
{
...
// -----------------------------------------------------------------------
void FatPlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void FatPlatePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float FatPlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void FatPlatePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatPlatePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatPlatePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatPlatePresetEngine::setCrossoverFreq (float hz)
{
...
void FatPlatePresetEngine::setModDepth (float depth)
{
...
void FatPlatePresetEngine::setNoiseModDepth (float samples)
{
...
void FatPlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void FatPlatePresetEngine::setSize (float size)
{
...
void FatPlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void FatPlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = scale; }

void FatPlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void FatPlatePresetEngine::setAirDampingScale (float scale)
{
...
void FatPlatePresetEngine::setSizeRange (float min, float max)
{
...
void FatPlatePresetEngine::setDecayBoost (float boost)
{
...
void FatPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void FatPlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void FatPlatePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void FatPlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void FatPlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void FatPlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class FatPlatePresetWrapper : public PresetEngineBase
{
public:
    FatPlatePresetWrapper() = default;
    ~FatPlatePresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new FatPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gFatPlatePresetRegistrar ("PresetFatPlate", &createFatPlatePreset);
