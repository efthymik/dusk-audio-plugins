...
//
// Per-preset reverb engine for "Fat Drums".
...
// any other preset. The class has been renamed to FatDrumsPresetEngine to avoid
// ODR conflicts.

#include "FatDrumsPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetFatDrums in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.831695f;
    constexpr float kBakedSizeRangeMax       = 2.495085f;
    constexpr float kBakedAirDampingScale    = 0.8f;
    constexpr float kBakedNoiseModDepth      = 8.0f;
    constexpr float kBakedTrebleMultScale    = 0.77f;
    ...
    constexpr float kVvAirDampingScale   = 0.764957f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 1.66339f — size range now absolute
    constexpr float kVvTiltLowDb         = -0.364335f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -0.565808f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.900724f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.510232f
    ...
    // Max correction magnitude for this preset: 4.12 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -0.345306f, -1.15544f, -2.19944f, -2.05082f, -2.26559f, -1.22854f, -1.46519f, -2.04469f, -0.274044f, -2.80295f, -5.86956f, -1.07991f };
...
class FatDrumsPresetEngine
{
public:
    FatDrumsPresetEngine();
...
void FatDrumsPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void FatDrumsPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float FatDrumsPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void FatDrumsPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void FatDrumsPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
FatDrumsPresetEngine::FatDrumsPresetEngine()
{
...
// -----------------------------------------------------------------------
void FatDrumsPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void FatDrumsPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float FatDrumsPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void FatDrumsPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatDrumsPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatDrumsPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void FatDrumsPresetEngine::setCrossoverFreq (float hz)
{
...
void FatDrumsPresetEngine::setModDepth (float depth)
{
...
void FatDrumsPresetEngine::setNoiseModDepth (float samples)
{
...
void FatDrumsPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void FatDrumsPresetEngine::setSize (float size)
{
...
void FatDrumsPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void FatDrumsPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void FatDrumsPresetEngine::setHighCrossoverFreq (float hz)
{
...
void FatDrumsPresetEngine::setAirDampingScale (float scale)
{
...
void FatDrumsPresetEngine::setSizeRange (float min, float max)
{
...
void FatDrumsPresetEngine::setDecayBoost (float boost)
{
...
void FatDrumsPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void FatDrumsPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void FatDrumsPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void FatDrumsPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void FatDrumsPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void FatDrumsPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class FatDrumsPresetWrapper : public PresetEngineBase
{
public:
    FatDrumsPresetWrapper() = default;
    ~FatDrumsPresetWrapper() override = default;
    ...
    void prepare (double sampleRate, int maxBlockSize) override
    {
        ...
        directOnsetSamples_ = 0.0f;
        directOnsetEnv_ = 0.0f;
    ...
    const char* getPresetName() const override { return "Fat Drums"; }
...
private:
    ...
    // Direct path onset injection state
    float directOnsetSamples_ = 0.0f;
    float directOnsetEnv_ = 0.0f;
    // PATCH_POINT_MEMBERS
};
...
{
    return std::unique_ptr<PresetEngineBase> (new FatDrumsPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gFatDrumsPresetRegistrar ("PresetFatDrums", &createFatDrumsPreset);
