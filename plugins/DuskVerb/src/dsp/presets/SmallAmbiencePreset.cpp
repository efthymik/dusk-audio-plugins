...
//
// Per-preset reverb engine for "Small Ambience".
...
// any other preset. The class has been renamed to SmallAmbiencePresetEngine to avoid
// ODR conflicts.

#include "SmallAmbiencePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetSmallAmbience in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.397664f;
    constexpr float kBakedSizeRangeMax       = 1.192992f;
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 1.14f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.895832f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.795328f — size range now absolute
    constexpr float kVvTiltLowDb         = 0.384729f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -3.77877f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.75f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 1.26948f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 1.06238f, -2.31541f, -1.88413f, -1.0629f, -2.35042f, -0.0820683f, -3.54056f, 1.89911f, 0.556677f, -0.585431f, -2.76728f, 12.0f };
    ...
    // Duration: 288ms, 64 points.
    // -----------------------------------------------------------------
    constexpr float kOnsetDurationMs = 288.0f;
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f,
        0.7606f, 0.5586f, 0.4746f, 0.3425f, 0.3699f, 0.3312f, 0.3883f, 0.3873f,
        0.4200f, 0.3940f, 0.3166f, 0.3398f, 0.3929f, 0.4478f, 0.6037f, 1.0000f,
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.8158f, 0.6265f, 0.4489f, 0.3323f,
        0.2906f, 0.4278f, 0.5336f, 0.6147f, 0.7232f, 0.9324f, 0.8100f, 0.6071f,
        0.4999f, 0.3917f, 0.4117f, 0.3868f, 0.4167f, 0.4234f, 0.4632f, 0.5748f,
        0.6200f, 0.8812f, 0.9747f, 0.8182f, 0.7115f, 0.8695f, 1.0000f, 1.0000f
    };
...
class SmallAmbiencePresetEngine
{
public:
    SmallAmbiencePresetEngine();
...
void SmallAmbiencePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void SmallAmbiencePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float SmallAmbiencePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void SmallAmbiencePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void SmallAmbiencePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
SmallAmbiencePresetEngine::SmallAmbiencePresetEngine()
{
...
// -----------------------------------------------------------------------
void SmallAmbiencePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void SmallAmbiencePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float SmallAmbiencePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void SmallAmbiencePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SmallAmbiencePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void SmallAmbiencePresetEngine::setTrebleMultiply (float mult)
{
...
void SmallAmbiencePresetEngine::setModDepth (float depth)
{
...
void SmallAmbiencePresetEngine::setNoiseModDepth (float samples)
{
...
void SmallAmbiencePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void SmallAmbiencePresetEngine::setSize (float size)
{
...
void SmallAmbiencePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void SmallAmbiencePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void SmallAmbiencePresetEngine::setHighCrossoverFreq (float hz)
{
...
void SmallAmbiencePresetEngine::setAirDampingScale (float scale)
{
...
void SmallAmbiencePresetEngine::setDecayBoost (float boost)
{
...
void SmallAmbiencePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void SmallAmbiencePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void SmallAmbiencePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void SmallAmbiencePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void SmallAmbiencePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void SmallAmbiencePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class SmallAmbiencePresetWrapper : public PresetEngineBase
{
public:
    SmallAmbiencePresetWrapper() = default;
    ~SmallAmbiencePresetWrapper() override = default;
                  ...
                  float* outputL, float* outputR, int numSamples) override
    {
        ...
        // Fills spectral nulls in the QuadTank's comb-filtered onset.
        {
            constexpr float kNoiseLevel = 0.0008f;
            constexpr float kOnsetWindowMs = 200.0f;
            constexpr float kOnsetThresh = 0.005f;
            for (int i = 0; i < numSamples; ++i)
            {
    ...
    const char* getPresetName() const override { return "Small Ambience"; }
...
private:
    SmallAmbiencePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new SmallAmbiencePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gSmallAmbiencePresetRegistrar ("PresetSmallAmbience", &createSmallAmbiencePreset);
