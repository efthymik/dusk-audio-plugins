...
//
// Per-preset reverb engine for "Long Dark 70s Snare Room".
...
// any other preset. The class has been renamed to LongDark70sSnareRoomPresetEngine to avoid
// ODR conflicts.

#include "LongDark70sSnareRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetLongDark70sSnareRoom in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.389493f;
    constexpr float kBakedSizeRangeMax       = 1.168481f;
    ...
    constexpr float kVvAirDampingScale   = 0.874922f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.778987f — size range now absolute
    constexpr float kVvTiltLowDb         = -2.2577f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -5.29037f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 1.3087f
    ...
    // Max correction magnitude for this preset: 5.74 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 5.71862f, 3.96192f, 3.7354f, -1.48314f, 1.54599f, 2.48234f, 1.04503f, 2.87335f, 1.40311f, -4.97268f, 9.07354f, 12.0f };
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 0.8666f, 0.7865f, 0.6225f, 0.5380f, 0.5559f, 0.5604f, 0.5640f,
        0.5069f, 0.5143f, 0.6541f, 0.8292f, 0.8297f, 0.7128f, 0.7658f, 0.7615f,
        0.6863f, 0.5880f, 0.7507f, 0.9945f, 1.0000f, 1.0000f, 0.9081f, 0.8333f,
        0.5978f, 0.6486f, 0.8537f, 1.0000f, 1.0000f, 1.0000f, 0.9814f, 0.7187f,
        0.6760f, 0.7039f, 0.6593f, 0.8849f, 0.9450f, 1.0000f, 1.0000f, 0.9989f,
        0.8065f, 0.6775f, 0.5801f, 0.6844f, 0.8574f, 0.9966f, 0.8449f, 0.8139f,
        0.6750f, 0.8707f, 0.8864f, 0.9394f, 0.5987f, 0.6142f, 0.7051f, 1.0000f
    };
...
class LongDark70sSnareRoomPresetEngine
{
public:
    LongDark70sSnareRoomPresetEngine();
...
void LongDark70sSnareRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LongDark70sSnareRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LongDark70sSnareRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void LongDark70sSnareRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LongDark70sSnareRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
LongDark70sSnareRoomPresetEngine::LongDark70sSnareRoomPresetEngine()
{
...
// -----------------------------------------------------------------------
void LongDark70sSnareRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void LongDark70sSnareRoomPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float LongDark70sSnareRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void LongDark70sSnareRoomPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LongDark70sSnareRoomPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LongDark70sSnareRoomPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LongDark70sSnareRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void LongDark70sSnareRoomPresetEngine::setModDepth (float depth)
{
...
void LongDark70sSnareRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void LongDark70sSnareRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void LongDark70sSnareRoomPresetEngine::setSize (float size)
{
...
void LongDark70sSnareRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void LongDark70sSnareRoomPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void LongDark70sSnareRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void LongDark70sSnareRoomPresetEngine::setAirDampingScale (float scale)
{
...
void LongDark70sSnareRoomPresetEngine::setSizeRange (float min, float max)
{
...
void LongDark70sSnareRoomPresetEngine::setDecayBoost (float boost)
{
...
void LongDark70sSnareRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LongDark70sSnareRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LongDark70sSnareRoomPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void LongDark70sSnareRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LongDark70sSnareRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void LongDark70sSnareRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LongDark70sSnareRoomPresetWrapper : public PresetEngineBase
{
public:
    LongDark70sSnareRoomPresetWrapper() = default;
    ~LongDark70sSnareRoomPresetWrapper() override = default;
                  ...
                  float* outputL, float* outputR, int numSamples) override
    {
        ...
        // <<< PATCH_BODY_BEGIN:extend_tail_gain >>>
        {
            constexpr float kExtendTailGain = 1.412538f;
            constexpr float kSafetyCeiling = 32.0f;
            for (int i = 0; i < numSamples; ++i)
            {
                float l = outputL[i] * kExtendTailGain;
                float r = outputR[i] * kExtendTailGain;
                outputL[i] = std::clamp (l, -kSafetyCeiling, kSafetyCeiling);
                outputR[i] = std::clamp (r, -kSafetyCeiling, kSafetyCeiling);
            }
        }
        // <<< PATCH_BODY_END:extend_tail_gain >>>
    ...
    const char* getPresetName() const override { return "Long Dark 70s Snare Room"; }
...
private:
    LongDark70sSnareRoomPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new LongDark70sSnareRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLongDark70sSnareRoomPresetRegistrar ("PresetLongDark70sSnareRoom", &createLongDark70sSnareRoomPreset);
