...
//
// Per-preset reverb engine for "Large Ambience".
...
// any other preset. The class has been renamed to LargeAmbiencePresetEngine to avoid
// ODR conflicts.

#include "LargeAmbiencePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetLargeAmbience in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.69198f;
    constexpr float kBakedSizeRangeMax       = 2.07594f;
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.9f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.826268f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 1.38396f — size range now absolute
    constexpr float kVvTiltLowDb         = -0.625394f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -3.99319f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.886701f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.675857f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 2.15749f, 0.572348f, -4.17075f, 0.505193f, -0.749944f, 0.108104f, -1.23724f, 0.1429f, -0.128605f, -1.5406f, 3.77685f, 24.0f };
...
class LargeAmbiencePresetEngine
{
public:
    LargeAmbiencePresetEngine();
...
void LargeAmbiencePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LargeAmbiencePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LargeAmbiencePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void LargeAmbiencePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LargeAmbiencePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
LargeAmbiencePresetEngine::LargeAmbiencePresetEngine()
{
...
// -----------------------------------------------------------------------
void LargeAmbiencePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void LargeAmbiencePresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float LargeAmbiencePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void LargeAmbiencePresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeAmbiencePresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeAmbiencePresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeAmbiencePresetEngine::setCrossoverFreq (float hz)
{
...
void LargeAmbiencePresetEngine::setModDepth (float depth)
{
...
void LargeAmbiencePresetEngine::setNoiseModDepth (float samples)
{
...
void LargeAmbiencePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void LargeAmbiencePresetEngine::setSize (float size)
{
...
void LargeAmbiencePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void LargeAmbiencePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void LargeAmbiencePresetEngine::setHighCrossoverFreq (float hz)
{
...
void LargeAmbiencePresetEngine::setAirDampingScale (float scale)
{
...
void LargeAmbiencePresetEngine::setSizeRange (float min, float max)
{
...
void LargeAmbiencePresetEngine::setDecayBoost (float boost)
{
...
void LargeAmbiencePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LargeAmbiencePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LargeAmbiencePresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
    ...
    // Restore the same LFO/PRNG seeds and phase offsets as prepare() so
    // clearBuffers() produces the same starting state as a fresh prepare().
    static constexpr uint32_t lfoSeeds[kNumTanks]   = { 0x12345678u, 0x87654321u, 0xABCDEF01u, 0x13579BDFu };
    static constexpr uint32_t noiseSeeds[kNumTanks]  = { 0xDEADBEEFu, 0xCAFEBABEu, 0xFEEDFACEu, 0xBAADF00Du };
    static constexpr float    phaseOffsets[kNumTanks] = { 0.0f, 1.5707963f, 3.1415927f, 4.7123890f };
    for (int t = 0; t < kNumTanks; ++t)
    {
        tanks_[t].lfoPhase = phaseOffsets[t];
        tanks_[t].lfoPRNG = lfoSeeds[t];
        tanks_[t].noiseState = noiseSeeds[t];
    }
}

// -----------------------------------------------------------------------
void LargeAmbiencePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LargeAmbiencePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void LargeAmbiencePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LargeAmbiencePresetWrapper : public PresetEngineBase
{
public:
    LargeAmbiencePresetWrapper() = default;
    ~LargeAmbiencePresetWrapper() override = default;
    ...
    void prepare (double sampleRate, int maxBlockSize) override
    {
        ...
        // Use cached override size range if present; otherwise use baked defaults.
    ...
    void setLateGainScale (float scale) override
    {
        ...
        engine_.setLateGainScale (std::max (scale, 0.0f) * kBakedLateGainScale);
    }
    ...
    const char* getPresetName() const override { return "Large Ambience"; }
...
private:
    LargeAmbiencePresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new LargeAmbiencePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLargeAmbiencePresetRegistrar ("PresetLargeAmbience", &createLargeAmbiencePreset);
