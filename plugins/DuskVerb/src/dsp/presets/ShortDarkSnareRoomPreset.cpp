...
// any other preset. The class has been renamed to ShortDarkSnareRoomPresetEngine to avoid
// ODR conflicts.

#include "ShortDarkSnareRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetShortDarkSnareRoom in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.211375f;
    constexpr float kBakedSizeRangeMax       = 0.634125f;
    constexpr float kBakedAirDampingScale    = 0.8f;
    constexpr float kBakedNoiseModDepth      = 3.0f;  // reduced to match VV modulation
    constexpr float kBakedTrebleMultScale    = 0.65f;
    constexpr float kBakedTrebleMultScaleMax = 1.5f;
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.76f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.965234f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.42275f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.24997f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -8.0f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.883239f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.53f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -1.7801f, 0.259086f, -2.41713f, -1.2818f, -3.95044f, 1.67362f, -0.819154f, -0.772665f, 0.10433f, -4.35587f, -6.50249f, 18.0f };
...
class ShortDarkSnareRoomPresetEngine
{
public:
    ShortDarkSnareRoomPresetEngine();
...
private:
    ...
    // Safety clamp (±32 ceiling), matches wrapper output ceiling
...
void ShortDarkSnareRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void ShortDarkSnareRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float ShortDarkSnareRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void ShortDarkSnareRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void ShortDarkSnareRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
ShortDarkSnareRoomPresetEngine::ShortDarkSnareRoomPresetEngine()
{
...
void ShortDarkSnareRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void ShortDarkSnareRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void ShortDarkSnareRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float ShortDarkSnareRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void ShortDarkSnareRoomPresetEngine::setDecayTime (float seconds)
{
...
void ShortDarkSnareRoomPresetEngine::setBassMultiply (float mult)
{
...
void ShortDarkSnareRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void ShortDarkSnareRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void ShortDarkSnareRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void ShortDarkSnareRoomPresetEngine::setAirDampingScale (float scale)
{
...
void ShortDarkSnareRoomPresetEngine::setModDepth (float depth)
{
...
void ShortDarkSnareRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void ShortDarkSnareRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void ShortDarkSnareRoomPresetEngine::setSize (float size)
{
...
void ShortDarkSnareRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void ShortDarkSnareRoomPresetEngine::setDelayScale (float scale)
{
...
void ShortDarkSnareRoomPresetEngine::setNoiseModDepth (float samples)
{
    ...
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
    float maxJitter = 32.0f * rateRatio;  // Match prepare() headroom allocation
    independentNoiseModDepth_ = std::clamp (samples * rateRatio, -1.0f, maxJitter);
}

void ShortDarkSnareRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void ShortDarkSnareRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void ShortDarkSnareRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void ShortDarkSnareRoomPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void ShortDarkSnareRoomPresetEngine::setSizeRange (float min, float max)
{
...
void ShortDarkSnareRoomPresetEngine::setDecayBoost (float boost)
{
...
void ShortDarkSnareRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void ShortDarkSnareRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void ShortDarkSnareRoomPresetEngine::clearBuffers()
{
...
void ShortDarkSnareRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void ShortDarkSnareRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void ShortDarkSnareRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class ShortDarkSnareRoomPresetWrapper : public PresetEngineBase
{
public:
    ShortDarkSnareRoomPresetWrapper() = default;
    ~ShortDarkSnareRoomPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Short Dark Snare Room"; }
...
private:
    ShortDarkSnareRoomPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new ShortDarkSnareRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gShortDarkSnareRoomPresetRegistrar ("PresetShortDarkSnareRoom", &createShortDarkSnareRoomPreset);
