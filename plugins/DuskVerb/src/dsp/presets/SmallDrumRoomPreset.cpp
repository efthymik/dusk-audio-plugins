...
// any other preset. The class has been renamed to SmallDrumRoomPresetEngine to avoid
// ODR conflicts.

#include "SmallDrumRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetSmallDrumRoom in AlgorithmConfig.h
    ...
    constexpr float kVvAirDampingScale   = 0.947591f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.537708f — size range now absolute
    constexpr float kVvTiltLowDb         = -3.07221f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -5.6881f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.909094f;
    ...
    // Increased from 0.226327f to lengthen tail for decay_ratio fix.
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.8f
    ...
    // -----------------------------------------------------------------
    // Mids damped + air band sustained — DV had +3-6 dB excess in 125Hz-8kHz
    // and -13 dB deficit at 12-20 kHz. Rebalances engine spectrum before EQ.
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        1.0000f, 0.9744f, 0.6299f, 0.4805f, 0.4460f, 0.3473f, 0.3034f, 0.2905f,
        0.3075f, 0.3952f, 0.3572f, 0.3581f, 0.3086f, 0.3181f, 0.2854f, 0.2800f,
        0.3411f, 0.4332f, 0.4280f, 0.3858f, 0.3530f, 0.4123f, 0.3872f, 0.4653f,
        0.4761f, 0.5333f, 0.4692f, 0.4167f, 0.3884f, 0.4619f, 0.5135f, 0.6278f,
        0.6615f, 0.6630f, 0.7099f, 0.7352f, 0.9796f, 0.9269f, 0.9972f, 0.8837f,
        0.6425f, 0.4952f, 0.4755f, 0.5699f, 0.5474f, 0.4894f, 0.5893f, 0.7061f,
        0.7251f, 0.7311f, 0.6026f, 0.6323f, 0.4794f, 0.4576f, 0.4489f, 0.6352f,
        0.8534f, 0.7544f, 0.5836f, 0.4666f, 0.6505f, 0.5923f, 0.5292f, 1.0000f
    };
...
class SmallDrumRoomPresetEngine
{
public:
    SmallDrumRoomPresetEngine();
...
private:
    ...
    float delayScaleAllocated_ = 1.0f; // delayScale_ at prepare() time — runtime max
...
void SmallDrumRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void SmallDrumRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float SmallDrumRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void SmallDrumRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void SmallDrumRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
SmallDrumRoomPresetEngine::SmallDrumRoomPresetEngine()
{
...
void SmallDrumRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
    ...
    // Recompute delay lengths and decay gains for the new base delays.
    if (prepared_)
    {
        updateDelayLengths();
...
void SmallDrumRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void SmallDrumRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float SmallDrumRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void SmallDrumRoomPresetEngine::setDecayTime (float seconds)
{
...
void SmallDrumRoomPresetEngine::setBassMultiply (float mult)
{
...
void SmallDrumRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void SmallDrumRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void SmallDrumRoomPresetEngine::setHighCrossoverFreq (float hz)
{
    highCrossoverFreq_ = std::max (hz, std::max (crossoverFreq_ + 1.0f, 1.0f));
    if (prepared_)
        updateDecayCoefficients();
}

void SmallDrumRoomPresetEngine::setAirDampingScale (float scale)
{
...
void SmallDrumRoomPresetEngine::setModDepth (float depth)
{
...
void SmallDrumRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void SmallDrumRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void SmallDrumRoomPresetEngine::setSize (float size)
{
...
void SmallDrumRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void SmallDrumRoomPresetEngine::setDelayScale (float scale)
{
    // Clamp to the prepared allocation ceiling so delay reads cannot wrap past buffer end
    float maxScale = prepared_ ? delayScaleAllocated_ : 4.0f;
    delayScale_ = std::clamp (scale, 0.25f, maxScale);
    if (prepared_)
    {
        updateDelayLengths();
        updateDecayCoefficients();
...
void SmallDrumRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void SmallDrumRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void SmallDrumRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void SmallDrumRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void SmallDrumRoomPresetEngine::setDecayBoost (float boost)
{
...
void SmallDrumRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void SmallDrumRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void SmallDrumRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void SmallDrumRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void SmallDrumRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class SmallDrumRoomPresetWrapper : public PresetEngineBase
{
public:
    SmallDrumRoomPresetWrapper() = default;
    ~SmallDrumRoomPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Small Drum Room"; }
...
private:
    SmallDrumRoomPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new SmallDrumRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gSmallDrumRoomPresetRegistrar ("PresetSmallDrumRoom", &createSmallDrumRoomPreset);
