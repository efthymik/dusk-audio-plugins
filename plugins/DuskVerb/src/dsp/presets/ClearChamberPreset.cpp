...
// any other preset. The class has been renamed to ClearChamberPresetEngine to avoid
// ODR conflicts.

#include "ClearChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetClearChamber in AlgorithmConfig.h
...
class ClearChamberPresetEngine
{
public:
    ClearChamberPresetEngine();
...
void ClearChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void ClearChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float ClearChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void ClearChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void ClearChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
ClearChamberPresetEngine::ClearChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void ClearChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void ClearChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float ClearChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void ClearChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void ClearChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void ClearChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void ClearChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void ClearChamberPresetEngine::setModDepth (float depth)
{
...
void ClearChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void ClearChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void ClearChamberPresetEngine::setSize (float size)
{
...
void ClearChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void ClearChamberPresetEngine::setAirDampingScale (float scale)
{
...
void ClearChamberPresetEngine::setSizeRange (float min, float max)
{
...
void ClearChamberPresetEngine::setDecayBoost (float boost)
{
...
void ClearChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void ClearChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void ClearChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void ClearChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void ClearChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void ClearChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class ClearChamberPresetWrapper : public PresetEngineBase
{
public:
    ClearChamberPresetWrapper() = default;
    ~ClearChamberPresetWrapper() override = default;
...
private:
    ClearChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new ClearChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gClearChamberPresetRegistrar ("PresetClearChamber", &createClearChamberPreset);
