...
//
// Per-preset reverb engine for "Med Ambience".
...
// any other preset. The class has been renamed to VocalChamberPresetEngine to avoid
// ODR conflicts.

#include "VocalChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetVocalChamber in AlgorithmConfig.h
    ...
    constexpr float kVvAirDampingScale   = 0.871706f;
    constexpr float kVvDelayScale        = 1.17639f;
    constexpr float kVvTiltLowDb         = -1.31147f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -3.24179f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.910015f;
    ...
    constexpr float kVvDecayTimeScale    = 1.26714f;
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 3.47393f, -3.03262f, 0.300123f, -1.03455f, -1.54747f, 0.384265f, -0.82722f, -0.483204f, -1.77527f, -6.36034f, 0.0f, 18.0f };
...
class VocalChamberPresetEngine
{
public:
    VocalChamberPresetEngine();
...
void VocalChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void VocalChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float VocalChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void VocalChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void VocalChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
VocalChamberPresetEngine::VocalChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void VocalChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void VocalChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float VocalChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void VocalChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void VocalChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void VocalChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void VocalChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void VocalChamberPresetEngine::setModDepth (float depth)
{
...
void VocalChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void VocalChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void VocalChamberPresetEngine::setSize (float size)
{
...
void VocalChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void VocalChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void VocalChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void VocalChamberPresetEngine::setAirDampingScale (float scale)
{
...
void VocalChamberPresetEngine::setSizeRange (float min, float max)
{
...
void VocalChamberPresetEngine::setDecayBoost (float boost)
{
...
void VocalChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void VocalChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void VocalChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void VocalChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void VocalChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void VocalChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class VocalChamberPresetWrapper : public PresetEngineBase
{
public:
    VocalChamberPresetWrapper() = default;
    ~VocalChamberPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Med Ambience"; }
                           ...
                           float* a1, float* a2, int maxBands) const override
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            else
            {
                ...
                // with kExtendTailGain folded into the numerator so the
                // exported correction matches process()'s final gain.
                constexpr float kExportTailGain = 1.412538f;
                b0[i] = shelfB0_ * kExportTailGain;
                b1[i] = shelfB1_ * kExportTailGain;
                b2[i] = 0.0f;
                a1[i] = shelfA1_;
                a2[i] = 0.0f;
            }
...
private:
    VocalChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new VocalChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gVocalChamberPresetRegistrar ("PresetVocalChamber", &createVocalChamberPreset);
