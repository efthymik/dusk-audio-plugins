...
// any other preset. The class has been renamed to DrumPlatePresetEngine to avoid
// ODR conflicts.

#include "DrumPlatePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetDrumPlate in AlgorithmConfig.h
    ...
    // -----------------------------------------------------------------
    // Structural constants — Drum Plate (short + sparkly for drums).
    // Target: EMT 140 "bright" damper, variant 1 → RT60 ~2.2s, centroid ~7.3 kHz.
    // Brighter and shorter than Vocal Plate; keeps more high-frequency energy.
    // -----------------------------------------------------------------
    constexpr float kVvCrossoverHz       = 1000.0f;   // bass/mid split
    constexpr float kVvHighCrossoverHz   = 7000.0f;   // push air split higher to let top through
    constexpr float kVvAirDampingScale   = 0.92f;     // light HF damping (plate retains sparkle)
    ...
    constexpr float kVvTiltHighDb        = 2.0f;      // small HF shelf boost to hit the 7.3 kHz target centroid
    constexpr float kVvTiltHighHz        = 7000.0f;
    constexpr float kVvStereoWidth       = 0.92f;     // slightly wider — drums benefit from spread
    constexpr float kVvDecayTimeScale    = 1.38f;     // same calibration as Vocal Plate (Dattorro underdelivers RT60)
...
class DrumPlatePresetEngine
{
public:
    DrumPlatePresetEngine();
...
void DrumPlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void DrumPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float DrumPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void DrumPlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void DrumPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
DrumPlatePresetEngine::DrumPlatePresetEngine()
{
...
void DrumPlatePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void DrumPlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void DrumPlatePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float DrumPlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void DrumPlatePresetEngine::setDecayTime (float seconds)
{
...
void DrumPlatePresetEngine::setBassMultiply (float mult)
{
...
void DrumPlatePresetEngine::setTrebleMultiply (float mult)
{
...
void DrumPlatePresetEngine::setAirDampingScale (float scale)
{
...
void DrumPlatePresetEngine::setModDepth (float depth)
{
...
void DrumPlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void DrumPlatePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void DrumPlatePresetEngine::setSize (float size)
{
...
void DrumPlatePresetEngine::setSoftOnsetMs (float ms)
{
...
void DrumPlatePresetEngine::setDelayScale (float scale)
{
...
void DrumPlatePresetEngine::setNoiseModDepth (float samples)
{
...
void DrumPlatePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void DrumPlatePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void DrumPlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void DrumPlatePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void DrumPlatePresetEngine::setSizeRange (float min, float max)
{
...
void DrumPlatePresetEngine::setDecayBoost (float boost)
{
...
void DrumPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void DrumPlatePresetEngine::clearBuffers()
{
...
void DrumPlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void DrumPlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void DrumPlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class DrumPlatePresetWrapper : public PresetEngineBase
{
public:
    DrumPlatePresetWrapper() = default;
    ~DrumPlatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Drum Plate"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new DrumPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gDrumPlatePresetRegistrar ("PresetDrumPlate", &createDrumPlatePreset);
