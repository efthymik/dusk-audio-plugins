...
namespace {
    ...
    constexpr float kBakedSizeRangeMin       = 0.78179f;
    constexpr float kBakedSizeRangeMax       = 2.34537f;
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 1.06f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.859667f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 1.56358f — size range now absolute
    constexpr float kVvTiltLowDb         = -0.717813f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -2.26904f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.904001f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.77073f
    ...
    // Max correction magnitude for this preset: 7.02 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -1.87603f, -2.79079f, -1.15689f, -3.11195f, -0.746509f, -3.83033f, -0.148508f, -2.06084f, 0.514329f, -1.92705f, -4.81116f, 15.28f };
...
void DrumPlatePresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }
