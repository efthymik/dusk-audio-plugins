...
// any other preset. The class has been renamed to VocalBoothPresetEngine to avoid
// ODR conflicts.

#include "VocalBoothPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetVocalBooth in AlgorithmConfig.h
...
class VocalBoothPresetEngine
{
public:
    VocalBoothPresetEngine();
...
void VocalBoothPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void VocalBoothPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float VocalBoothPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void VocalBoothPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void VocalBoothPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
VocalBoothPresetEngine::VocalBoothPresetEngine()
{
...
void VocalBoothPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void VocalBoothPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void VocalBoothPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float VocalBoothPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void VocalBoothPresetEngine::setDecayTime (float seconds)
{
...
void VocalBoothPresetEngine::setBassMultiply (float mult)
{
...
void VocalBoothPresetEngine::setTrebleMultiply (float mult)
{
...
void VocalBoothPresetEngine::setCrossoverFreq (float hz)
{
...
void VocalBoothPresetEngine::setHighCrossoverFreq (float hz)
{
...
void VocalBoothPresetEngine::setAirDampingScale (float scale)
{
...
void VocalBoothPresetEngine::setModDepth (float depth)
{
...
void VocalBoothPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void VocalBoothPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void VocalBoothPresetEngine::setSize (float size)
{
...
void VocalBoothPresetEngine::setSoftOnsetMs (float ms)
{
...
void VocalBoothPresetEngine::setDelayScale (float scale)
{
...
void VocalBoothPresetEngine::setNoiseModDepth (float samples)
{
...
void VocalBoothPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void VocalBoothPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void VocalBoothPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void VocalBoothPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void VocalBoothPresetEngine::setSizeRange (float min, float max)
{
...
void VocalBoothPresetEngine::setDecayBoost (float boost)
{
...
void VocalBoothPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void VocalBoothPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void VocalBoothPresetEngine::clearBuffers()
{
...
void VocalBoothPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void VocalBoothPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void VocalBoothPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class VocalBoothPresetWrapper : public PresetEngineBase
{
public:
    VocalBoothPresetWrapper() = default;
    ~VocalBoothPresetWrapper() override = default;
...
private:
    VocalBoothPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new VocalBoothPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gVocalBoothPresetRegistrar ("PresetVocalBooth", &createVocalBoothPreset);
