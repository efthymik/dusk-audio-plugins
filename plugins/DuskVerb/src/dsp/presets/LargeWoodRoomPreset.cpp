...
//
// Per-preset reverb engine for "Large Wood Room".
...
// any other preset. The class has been renamed to LargeWoodRoomPresetEngine to avoid
// ODR conflicts.

#include "LargeWoodRoomPreset.h"
...
namespace {
    ...
    constexpr float kVvStereoWidth       = 0.92168f;
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 2.16371f, -5.54286f, 4.32848f, -2.26795f, -1.18234f, -1.42818f, 0.41801f, 0.720704f, 1.17393f, -0.539472f, -8.80473f, 10.3284f };
    ...
    // Duration: 248ms, 64 points.
    // -----------------------------------------------------------------
    constexpr float kOnsetDurationMs = 248.0f;
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9842f, 0.9345f,
        0.8658f, 0.7784f, 0.6858f, 0.6769f, 0.7262f, 0.5837f, 0.6844f, 0.6325f,
        0.6404f, 0.6759f, 0.6743f, 0.7078f, 0.6904f, 0.6177f, 0.6547f, 0.7432f,
        0.9822f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9898f, 0.7626f,
        0.6222f, 0.6199f, 0.6073f, 0.6266f, 0.6274f, 0.8613f, 0.9728f, 1.0000f,
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9465f, 0.6022f,
        0.5634f, 0.4573f, 0.5173f, 0.5287f, 0.6404f, 0.9939f, 1.0000f, 1.0000f
    };
...
class LargeWoodRoomPresetEngine
{
public:
    LargeWoodRoomPresetEngine();
...
void LargeWoodRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LargeWoodRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LargeWoodRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void LargeWoodRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LargeWoodRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
LargeWoodRoomPresetEngine::LargeWoodRoomPresetEngine()
{
...
// -----------------------------------------------------------------------
void LargeWoodRoomPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float LargeWoodRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void LargeWoodRoomPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeWoodRoomPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeWoodRoomPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeWoodRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void LargeWoodRoomPresetEngine::setSize (float size)
{
...
void LargeWoodRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void LargeWoodRoomPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void LargeWoodRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void LargeWoodRoomPresetEngine::setAirDampingScale (float scale)
{
...
void LargeWoodRoomPresetEngine::setDecayBoost (float boost)
{
...
void LargeWoodRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LargeWoodRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LargeWoodRoomPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
    ...
    // Restore the same LFO/PRNG seeds and phase offsets as prepare()
...
// -----------------------------------------------------------------------
void LargeWoodRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LargeWoodRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
    float lowCrossoverCoeff = std::exp (-kTwoPi * crossoverFreq_ / sr);
    float highCrossoverCoeff = std::exp (-kTwoPi * highCrossoverFreq_ / sr);

    // Recompute the four explicit five-band crossover coefficients from the
    // baked Hz values so setCrossovers() can restore them after
    // computeGainsFromBase() (which would otherwise overwrite with derived values).
    float fbCoeffs[4];
    for (int b = 0; b < 4; ++b)
    ...
    for (int t = 0; t < kNumTanks; ++t)
    {
        ...
        float combinedMult[5] = {
            ...
            kFiveBandMult[2],                                   // mid band at natural rate
            kFiveBandMult[3] * trebleMultiply_,
            kFiveBandMult[4] * (trebleMultiply_ * airDampingScale_)
        };
...
void LargeWoodRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LargeWoodRoomPresetWrapper : public PresetEngineBase
{
public:
    LargeWoodRoomPresetWrapper() = default;
    ~LargeWoodRoomPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Large Wood Room"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new LargeWoodRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLargeWoodRoomPresetRegistrar ("PresetLargeWoodRoom", &createLargeWoodRoomPreset);
