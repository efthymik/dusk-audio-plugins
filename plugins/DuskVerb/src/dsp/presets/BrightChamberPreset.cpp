...
//
// Per-preset reverb engine for "Clear Chamber".
...
// any other preset. The class has been renamed to BrightChamberPresetEngine to avoid
// ODR conflicts.

#include "BrightChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetBrightChamber in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.401839f;
    constexpr float kBakedSizeRangeMax       = 1.205517f;
    ...
    constexpr float kVvAirDampingScale   = 0.916923f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.803678f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.12503f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -3.0f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.924751f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.687137f
    ...
    // Max correction magnitude for this preset: 2.34 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -2.4077f, 1.99259f, 1.81388f, -1.27309f, 0.351641f, -1.29309f, 0.281583f, -0.81322f, 1.98927f, 0.0730668f, -1.36041f, -0.704992f };
    ...
    // Duration: 286ms, 64 points.
    // -----------------------------------------------------------------
    constexpr float kOnsetDurationMs = 286.0f;
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f,
        0.8886f, 0.7105f, 0.6646f, 0.7322f, 0.9708f, 1.0000f, 1.0000f, 1.0000f,
        1.0000f, 0.9000f, 0.8321f, 0.7414f, 0.7809f, 0.7448f, 0.7873f, 0.7374f,
        0.6946f, 0.7679f, 0.8094f, 0.8994f, 0.6879f, 0.6288f, 0.7004f, 0.9591f,
        1.0000f, 0.9984f, 0.9933f, 1.0000f, 1.0000f, 1.0000f, 0.9654f, 0.7210f,
        0.6442f, 0.5831f, 0.6136f, 0.5966f, 0.6460f, 0.6516f, 0.5351f, 0.6405f,
        0.7902f, 1.0000f, 1.0000f, 0.9590f, 0.8960f, 0.9306f, 0.9865f, 1.0000f
    };
...
class BrightChamberPresetEngine
{
public:
    BrightChamberPresetEngine();
...
private:
    ...
    struct Tank
    {
        ...
        // Cached jitter values — held during freeze so the PRNG doesn't
        // advance silently (output would be zeroed anyway).
...
void BrightChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void BrightChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float BrightChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void BrightChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void BrightChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
BrightChamberPresetEngine::BrightChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void BrightChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void BrightChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;

    const float effectiveNoiseMod = frozen_ ? 0.0f
                                  : ((independentNoiseModDepth_ >= 0.0f)
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        for (int t = 0; t < kNumTanks; ++t)
        {
            ...
            // Freeze preserves the cached jitter so the PRNG doesn't advance
            // silently (output would be zero anyway via effectiveNoiseMod).
            float jitter1 = frozen_ ? tank.lastJitter1
                                    : nextDrift (tank.noiseState) * effectiveNoiseMod;
            if (! frozen_) tank.lastJitter1 = jitter1;
            ...
            float jitter2 = frozen_ ? tank.lastJitter2
                                    : nextDrift (tank.noiseState) * effectiveNoiseMod;
            if (! frozen_) tank.lastJitter2 = jitter2;
...
// -----------------------------------------------------------------------
float BrightChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void BrightChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void BrightChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void BrightChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void BrightChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void BrightChamberPresetEngine::setModDepth (float depth)
{
...
void BrightChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void BrightChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void BrightChamberPresetEngine::setSize (float size)
{
...
void BrightChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void BrightChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void BrightChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void BrightChamberPresetEngine::setAirDampingScale (float scale)
{
...
void BrightChamberPresetEngine::setSizeRange (float min, float max)
{
...
void BrightChamberPresetEngine::setDecayBoost (float boost)
{
...
void BrightChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void BrightChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void BrightChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void BrightChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void BrightChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void BrightChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class BrightChamberPresetWrapper : public PresetEngineBase
{
public:
    BrightChamberPresetWrapper() = default;
    ~BrightChamberPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Clear Chamber"; }
...
private:
    BrightChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new BrightChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gBrightChamberPresetRegistrar ("PresetBrightChamber", &createBrightChamberPreset);
