...
//
// Per-preset reverb engine for "Medium Gate".
...
// any other preset. The class has been renamed to ReversePresetEngine to avoid
// ODR conflicts.

#include "ReversePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetReverse in AlgorithmConfig.h
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.96f — bass knob now shows real multiplier
    ...
    constexpr float kVvTiltLowDb         = 0.927287f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = 0.0f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.920478f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.831814f
    ...
    // Max correction magnitude for this preset: 5.27 dB
    // -----------------------------------------------------------------
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -10.5244f, -14.13f, 12.0f, -12.0f, -12.0f, -10.6335f, -12.0f, 9.16389f, 5.76038f, 0.15928f, 0.50979f, 15.98f };
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        1.0000f, 1.0000f, 0.9648f, 1.0000f, 0.9057f, 1.0000f, 1.0000f, 1.0000f,
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.8598f, 0.6169f, 0.5611f, 0.5308f,
        0.5431f, 0.4364f, 0.3863f, 0.4602f, 0.5007f, 0.5652f, 0.6894f, 0.7914f,
        0.7283f, 0.6070f, 0.5608f, 0.5051f, 0.5515f, 0.6351f, 0.7550f, 0.6983f,
        0.7901f, 0.7221f, 0.6660f, 0.5333f, 0.8004f, 0.9561f, 0.9892f, 0.9225f,
        0.6786f, 0.5826f, 0.5767f, 0.6632f, 0.7677f, 0.9017f, 1.0000f, 1.0000f,
        0.9394f, 0.9824f, 0.9337f, 0.9185f, 0.8190f, 0.8941f, 1.0000f, 1.0000f,
        0.8508f, 0.6671f, 0.5623f, 0.6859f, 0.5883f, 0.7043f, 0.6319f, 1.0000f
    };
...
void ReversePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void ReversePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float ReversePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void ReversePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void ReversePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
ReversePresetEngine::ReversePresetEngine()
{
...
void ReversePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void ReversePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void ReversePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float ReversePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void ReversePresetEngine::setDecayTime (float seconds)
{
...
void ReversePresetEngine::setBassMultiply (float mult)
{
...
void ReversePresetEngine::setTrebleMultiply (float mult)
{
...
void ReversePresetEngine::setCrossoverFreq (float hz)
{
...
void ReversePresetEngine::setHighCrossoverFreq (float hz)
{
    highCrossoverFreq_ = std::max (hz, std::max (100.0f, crossoverFreq_ + 1.0f));
    if (prepared_)
        updateDecayCoefficients();
}

void ReversePresetEngine::setAirDampingScale (float scale)
{
...
void ReversePresetEngine::setModDepth (float depth)
{
...
void ReversePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void ReversePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
...
void ReversePresetEngine::setSize (float size)
{
...
void ReversePresetEngine::setSoftOnsetMs (float ms)
{
...
void ReversePresetEngine::setDelayScale (float scale)
{
...
void ReversePresetEngine::setNoiseModDepth (float samples)
{
...
void ReversePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void ReversePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void ReversePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void ReversePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void ReversePresetEngine::setSizeRange (float min, float max)
{
...
void ReversePresetEngine::setDecayBoost (float boost)
{
...
void ReversePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void ReversePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void ReversePresetEngine::clearBuffers()
{
...
void ReversePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void ReversePresetEngine::updateDecayCoefficients()
{
...
void ReversePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class ReversePresetWrapper : public PresetEngineBase
{
public:
    ReversePresetWrapper() = default;
    ~ReversePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Medium Gate"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new ReversePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gReversePresetRegistrar ("PresetReverse", &createReversePreset);
