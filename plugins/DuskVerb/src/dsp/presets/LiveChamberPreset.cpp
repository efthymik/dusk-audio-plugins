...
// any other preset. The class has been renamed to LiveChamberPresetEngine to avoid
// ODR conflicts.

#include "LiveChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetLiveChamber in AlgorithmConfig.h
...
class LiveChamberPresetEngine
{
public:
    LiveChamberPresetEngine();
...
void LiveChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LiveChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LiveChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void LiveChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LiveChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
LiveChamberPresetEngine::LiveChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void LiveChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void LiveChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float LiveChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void LiveChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LiveChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LiveChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LiveChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void LiveChamberPresetEngine::setModDepth (float depth)
{
...
void LiveChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void LiveChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void LiveChamberPresetEngine::setSize (float size)
{
...
void LiveChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void LiveChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void LiveChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void LiveChamberPresetEngine::setAirDampingScale (float scale)
{
...
void LiveChamberPresetEngine::setSizeRange (float min, float max)
{
...
void LiveChamberPresetEngine::setDecayBoost (float boost)
{
...
void LiveChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LiveChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LiveChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void LiveChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LiveChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void LiveChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LiveChamberPresetWrapper : public PresetEngineBase
{
public:
    LiveChamberPresetWrapper() = default;
    ~LiveChamberPresetWrapper() override = default;
...
private:
    LiveChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new LiveChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLiveChamberPresetRegistrar ("PresetLiveChamber", &createLiveChamberPreset);
