...
// any other preset. The class has been renamed to CrossStickRoomPresetEngine to avoid
// ODR conflicts.

#include "CrossStickRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetCrossStickRoom in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.219446f;
    constexpr float kBakedSizeRangeMax       = 0.658339f;
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.79f — bass knob now shows real multiplier
    ...
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.438893f — size range now absolute
    constexpr float kVvTiltLowDb         = -0.0319965f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -3.4784f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.90026f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.389899f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -18.0f, -15.81f, -2.31f, -17.64f, -4.76f, -19.81f, -3.02f, -4.62f, -1.19317f, -1.14f, -7.34f, 0.0f };
    ...
    // -----------------------------------------------------------------
    constexpr float kFiveBandMult[5] = { 1.00f, 1.00f, 1.00f, 1.00f, 0.40f };
...
class CrossStickRoomPresetEngine
{
public:
    CrossStickRoomPresetEngine();
...
void CrossStickRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void CrossStickRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float CrossStickRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void CrossStickRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void CrossStickRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
CrossStickRoomPresetEngine::CrossStickRoomPresetEngine()
{
...
void CrossStickRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void CrossStickRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
    ...
    // Seed modulation state only on first prepare — re-preparation (e.g. sample
    // rate change) should not reset LFO/PRNG state, which would cause audible
...
void CrossStickRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float CrossStickRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void CrossStickRoomPresetEngine::setDecayTime (float seconds)
{
...
void CrossStickRoomPresetEngine::setBassMultiply (float mult)
{
...
void CrossStickRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void CrossStickRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void CrossStickRoomPresetEngine::setHighCrossoverFreq (float hz)
{
    highCrossoverFreq_ = std::max (hz, crossoverFreq_ + 1.0f);
    if (prepared_)
        updateDecayCoefficients();
}

void CrossStickRoomPresetEngine::setAirDampingScale (float scale)
{
...
void CrossStickRoomPresetEngine::setModDepth (float depth)
{
...
void CrossStickRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void CrossStickRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void CrossStickRoomPresetEngine::setSize (float size)
{
...
void CrossStickRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void CrossStickRoomPresetEngine::setDelayScale (float scale)
{
...
void CrossStickRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void CrossStickRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void CrossStickRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void CrossStickRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void CrossStickRoomPresetEngine::setDecayBoost (float boost)
{
...
void CrossStickRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void CrossStickRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void CrossStickRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void CrossStickRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
class CrossStickRoomPresetWrapper : public PresetEngineBase
{
public:
    CrossStickRoomPresetWrapper() = default;
    ~CrossStickRoomPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Cross Stick Room"; }
...
private:
    CrossStickRoomPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new CrossStickRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gCrossStickRoomPresetRegistrar ("PresetCrossStickRoom", &createCrossStickRoomPreset);
