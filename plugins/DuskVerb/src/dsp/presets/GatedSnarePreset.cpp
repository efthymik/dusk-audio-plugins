...
// any other preset. The class has been renamed to GatedSnarePresetEngine to avoid
// ODR conflicts.

#include "GatedSnarePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetGatedSnare in AlgorithmConfig.h
    ...
    constexpr float kBakedTrebleMultScale    = 0.71f;
    constexpr float kBakedTrebleMultScaleMax = 1.5f;
    constexpr float kBakedBassMultScale      = 1.12f;
    ...
    // Post-engine safety ceiling — must match engine's kSafetyClip for
    // consistent clamping after corrective EQ + stereo width processing.
    constexpr float kWrapperSafetyClip       = 4.0f;
    ...
    // -----------------------------------------------------------------
    constexpr float kVvBassMultiply      = 1.0f;
    constexpr float kVvTrebleMultiply    = 0.903317f;
    ...
    constexpr float kVvTiltLowDb         = -0.41208f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -4.03931f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.894987f;
    ...
    constexpr float kVvDecayTimeScale    = 0.528398f;
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -3.61751f, -9.64368f, -6.02399f, 3.79306f, 12.0f, 12.0f, 12.0f, 12.0f, 12.0f, 12.0f, 12.0f, 12.0f };
    ...
    // Duration: 298ms, 64 points.
    // -----------------------------------------------------------------
    constexpr float kOnsetDurationMs = 298.0f;
    constexpr int kOnsetTableSize = 64;
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        1.0000f, 0.8237f, 0.3452f, 0.2839f, 0.4539f, 1.0000f, 1.0000f, 1.0000f,
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.6859f, 0.6500f, 0.7000f,
        0.7898f, 0.7723f, 0.8360f, 0.9876f, 1.0000f, 0.9628f, 0.7706f, 0.8166f,
        0.6664f, 0.6669f, 0.6976f, 0.6245f, 0.7199f, 0.5744f, 0.6548f, 0.5730f,
        0.7050f, 0.9501f, 1.0000f, 1.0000f, 1.0000f, 0.9345f, 0.7052f, 0.6193f,
        0.6788f, 0.9165f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9865f, 0.9324f,
        0.7958f, 0.7404f, 0.6790f, 0.7118f, 0.6803f, 0.7229f, 0.9154f, 1.0000f,
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9894f, 0.9753f, 1.0000f
    };
...
class GatedSnarePresetEngine
{
public:
    GatedSnarePresetEngine();
...
void GatedSnarePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void GatedSnarePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float GatedSnarePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void GatedSnarePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void GatedSnarePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
GatedSnarePresetEngine::GatedSnarePresetEngine()
{
...
void GatedSnarePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void GatedSnarePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void GatedSnarePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float GatedSnarePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void GatedSnarePresetEngine::setDecayTime (float seconds)
{
...
void GatedSnarePresetEngine::setBassMultiply (float mult)
{
...
void GatedSnarePresetEngine::setTrebleMultiply (float mult)
{
...
void GatedSnarePresetEngine::setHighCrossoverFreq (float hz)
{
...
void GatedSnarePresetEngine::setAirDampingScale (float scale)
{
...
void GatedSnarePresetEngine::setModDepth (float depth)
{
...
void GatedSnarePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void GatedSnarePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void GatedSnarePresetEngine::setSize (float size)
{
...
void GatedSnarePresetEngine::setSoftOnsetMs (float ms)
{
...
void GatedSnarePresetEngine::setDelayScale (float scale)
{
...
void GatedSnarePresetEngine::setNoiseModDepth (float samples)
{
...
void GatedSnarePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void GatedSnarePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void GatedSnarePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void GatedSnarePresetEngine::setLateGainScale (float scale)
{
...
void GatedSnarePresetEngine::setDecayBoost (float boost)
{
...
void GatedSnarePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void GatedSnarePresetEngine::clearBuffers()
{
...
void GatedSnarePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void GatedSnarePresetEngine::updateDecayCoefficients()
{
    ...
    auto updateTankDamping = [&] (Tank& tank)
    {
        ...
        float combinedMult[5] = {
            ...
            kFiveBandMult[4] * std::max (trebleMultiply_ * airDampingScale_, 0.01f)
        };
...
void GatedSnarePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class GatedSnarePresetWrapper : public PresetEngineBase
{
public:
    GatedSnarePresetWrapper() = default;
    ~GatedSnarePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Gated Snare"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new GatedSnarePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gGatedSnarePresetRegistrar ("PresetGatedSnare", &createGatedSnarePreset);
