...
// any other preset. The class has been renamed to TightRoomPresetEngine to avoid
// ODR conflicts.

#include "TightRoomPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetTightRoom in AlgorithmConfig.h
...
class TightRoomPresetEngine
{
public:
    TightRoomPresetEngine();
...
void TightRoomPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void TightRoomPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float TightRoomPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void TightRoomPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void TightRoomPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
TightRoomPresetEngine::TightRoomPresetEngine()
{
...
void TightRoomPresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void TightRoomPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void TightRoomPresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float TightRoomPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void TightRoomPresetEngine::setDecayTime (float seconds)
{
...
void TightRoomPresetEngine::setBassMultiply (float mult)
{
...
void TightRoomPresetEngine::setTrebleMultiply (float mult)
{
...
void TightRoomPresetEngine::setCrossoverFreq (float hz)
{
...
void TightRoomPresetEngine::setHighCrossoverFreq (float hz)
{
...
void TightRoomPresetEngine::setAirDampingScale (float scale)
{
...
void TightRoomPresetEngine::setModDepth (float depth)
{
...
void TightRoomPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void TightRoomPresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
...
void TightRoomPresetEngine::setSize (float size)
{
...
void TightRoomPresetEngine::setSoftOnsetMs (float ms)
{
...
void TightRoomPresetEngine::setDelayScale (float scale)
{
...
void TightRoomPresetEngine::setNoiseModDepth (float samples)
{
...
void TightRoomPresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void TightRoomPresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void TightRoomPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void TightRoomPresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void TightRoomPresetEngine::setSizeRange (float min, float max)
{
...
void TightRoomPresetEngine::setDecayBoost (float boost)
{
...
void TightRoomPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void TightRoomPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void TightRoomPresetEngine::clearBuffers()
{
...
void TightRoomPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void TightRoomPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void TightRoomPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class TightRoomPresetWrapper : public PresetEngineBase
{
public:
    TightRoomPresetWrapper() = default;
    ~TightRoomPresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new TightRoomPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gTightRoomPresetRegistrar ("PresetTightRoom", &createTightRoomPreset);
