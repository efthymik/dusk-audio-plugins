...
//
// Per-preset reverb engine for "Large Chamber".
...
// any other preset. The class has been renamed to LargeChamberPresetEngine to avoid
// ODR conflicts.

#include "LargeChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetLargeChamber in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.358791f;
    constexpr float kBakedSizeRangeMax       = 1.076373f;
    ...
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.91f — bass knob now shows real multiplier
    ...
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.717582f — size range now absolute
    constexpr float kVvTiltLowDb         = -1.21994f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -2.5404f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.89536f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.73f
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 1.0000f, 0.9838f, 0.8945f,
        0.6704f, 0.6265f, 0.6057f, 0.6530f, 0.6564f, 0.7888f, 0.8461f, 0.9378f,
        0.8183f, 0.6951f, 0.7661f, 0.8158f, 1.0000f, 1.0000f, 1.0000f, 1.0000f,
        0.9919f, 0.7970f, 0.6548f, 0.4945f, 0.5132f, 0.6128f, 0.8841f, 1.0000f,
        1.0000f, 0.9619f, 0.9567f, 0.6848f, 0.7759f, 0.9020f, 1.0000f, 0.8534f,
        0.6828f, 0.7725f, 0.9813f, 0.9983f, 0.8444f, 0.7349f, 0.6788f, 0.6931f,
        0.6304f, 0.6403f, 0.5724f, 0.5903f, 0.6538f, 0.7782f, 0.9891f, 1.0000f
    };
...
class LargeChamberPresetEngine
{
public:
    LargeChamberPresetEngine();
...
void LargeChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void LargeChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float LargeChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void LargeChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void LargeChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
LargeChamberPresetEngine::LargeChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void LargeChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void LargeChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float LargeChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void LargeChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void LargeChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void LargeChamberPresetEngine::setModDepth (float depth)
{
...
void LargeChamberPresetEngine::setNoiseModDepth (float samples)
{
...
void LargeChamberPresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_) updateLFORates();
}

void LargeChamberPresetEngine::setSize (float size)
{
...
void LargeChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void LargeChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void LargeChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void LargeChamberPresetEngine::setAirDampingScale (float scale)
{
...
void LargeChamberPresetEngine::setSizeRange (float min, float max)
{
...
void LargeChamberPresetEngine::setDecayBoost (float boost)
{
...
void LargeChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void LargeChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void LargeChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void LargeChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void LargeChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void LargeChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class LargeChamberPresetWrapper : public PresetEngineBase
{
public:
    LargeChamberPresetWrapper() = default;
    ~LargeChamberPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Large Chamber"; }
...
private:
    LargeChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new LargeChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gLargeChamberPresetRegistrar ("PresetLargeChamber", &createLargeChamberPreset);
