// Per-preset reverb engine for "Infinite".
...
// any other preset. The class has been renamed to InfinitePresetEngine to avoid
// ODR conflicts.

#include "InfinitePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetInfinite in AlgorithmConfig.h
    ...
    constexpr float kVvDecayTimeScale    = 0.181474f;
...
class InfinitePresetEngine
{
public:
    InfinitePresetEngine();
...
void InfinitePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void InfinitePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float InfinitePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void InfinitePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void InfinitePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
InfinitePresetEngine::InfinitePresetEngine()
{
...
void InfinitePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void InfinitePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void InfinitePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float InfinitePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void InfinitePresetEngine::setDecayTime (float seconds)
{
...
void InfinitePresetEngine::setBassMultiply (float mult)
{
...
void InfinitePresetEngine::setTrebleMultiply (float mult)
{
...
void InfinitePresetEngine::setCrossoverFreq (float hz)
{
...
void InfinitePresetEngine::setHighCrossoverFreq (float hz)
{
...
void InfinitePresetEngine::setAirDampingScale (float scale)
{
...
void InfinitePresetEngine::setModDepth (float depth)
{
...
void InfinitePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void InfinitePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void InfinitePresetEngine::setSize (float size)
{
...
void InfinitePresetEngine::setSoftOnsetMs (float ms)
{
...
void InfinitePresetEngine::setDelayScale (float scale)
{
...
void InfinitePresetEngine::setNoiseModDepth (float samples)
{
...
void InfinitePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void InfinitePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void InfinitePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void InfinitePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void InfinitePresetEngine::setSizeRange (float min, float max)
{
...
void InfinitePresetEngine::setDecayBoost (float boost)
{
...
void InfinitePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void InfinitePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void InfinitePresetEngine::clearBuffers()
{
...
void InfinitePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void InfinitePresetEngine::updateDecayCoefficients()
{
...
void InfinitePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class InfinitePresetWrapper : public PresetEngineBase
{
public:
    InfinitePresetWrapper() = default;
    ~InfinitePresetWrapper() override = default;
...
{
    return std::unique_ptr<PresetEngineBase> (new InfinitePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gInfinitePresetRegistrar ("PresetInfinite", &createInfinitePreset);
