...
//
// Per-preset reverb engine for "Vox Plate".
...
// any other preset. The class has been renamed to VoxPlatePresetEngine to avoid
// ODR conflicts.

#include "VoxPlatePreset.h"
#include "PresetEngineRegistry.h"

#include "../TwoBandDamping.h"
#include "VvPresetTaps.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetVoxPlate in AlgorithmConfig.h
    ...
    constexpr float kBakedModDepthScale      = 0.75f;  // VoxPlate: baseline LFO
    ...
    constexpr float        kVvDelayScale        = 1.0f;  // VoxPlate: keep baseline (halving delays made modes worse)
    ...
    constexpr float kVvDecayTimeScale    = 3.0f;  // VoxPlate: back to proven state
    ...
    // VoxPlate: neutral EQ (was DrumAir's -12dB cuts + +24dB at 15.5kHz — produced audible 15kHz spike).
...
class VoxPlatePresetEngine
{
public:
    VoxPlatePresetEngine();
...
private:
    ...
    // VV-derived Early Reflection tap bank — runs in parallel with the tank.
    // Reproduces VV's first ~150 ms echo pattern exactly (tap positions + gains
    // extracted from 50-take averaged canonical IR). Tank provides late tail.
    std::vector<float> erBufL_;
    std::vector<float> erBufR_;
    int erWritePos_ = 0;
    int erMask_ = 0;
    const dusk::baked::PresetErTapEntry* erEntry_ = nullptr;
    // per-tap cached delay-in-samples at current sr (avoids recompute per sample)
    std::vector<int> erTapDelays_;
    // ER taps reproduce VV's first 150 ms exactly at erMix=1.
    // Tank's bare output is at much higher level than VV's IR. Reducing
    // tankMix lets ER dominate the early region while tank still provides tail.
    float erMix_ = 1.0f;
    float tankMix_ = 0.10f;
...
void VoxPlatePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void VoxPlatePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float VoxPlatePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void VoxPlatePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void VoxPlatePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
VoxPlatePresetEngine::VoxPlatePresetEngine()
{
...
void VoxPlatePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void VoxPlatePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
    ...
    // ─── ER tap bank init ──────────────────────────────────────────────
    erEntry_ = dusk::baked::findErTapsForPreset ("Vox Plate");
    if (erEntry_ != nullptr && erEntry_->tapCount > 0)
    {
        const float rateRatio = static_cast<float> (sampleRate_ / 44100.0);
        int maxDelay = 0;
        erTapDelays_.resize (erEntry_->tapCount);
        for (int t = 0; t < erEntry_->tapCount; ++t)
        {
            int d = static_cast<int> (
                std::ceil (erEntry_->taps[t].delaySamples44k * rateRatio));
            erTapDelays_[t] = d;
            if (d > maxDelay) maxDelay = d;
        }
        int size = 1;
        while (size < maxDelay + 4) size <<= 1;
        erBufL_.assign (static_cast<size_t> (size), 0.0f);
        erBufR_.assign (static_cast<size_t> (size), 0.0f);
        erMask_ = size - 1;
        erWritePos_ = 0;
    }
}
...
void VoxPlatePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        constexpr float kOutputScale = 0.62f;  // VoxPlate: loudness match (EQ is neutral now)
        const float outputGain = kOutputScale * lateGainScale_;

        float scaledL = outL * outputGain;
        float scaledR = outR * outputGain;

        // ─── ER tap bank (parallel with tank) ──────────────────────────
        // Reproduces VV's first ~150 ms echo pattern exactly. Runs BEFORE
        // softOnset + limiter so that:
        //   - Freeze mode stops recording new input into the ER delay line
        //     (otherwise frozen tails would keep generating new ER from
        //     live signal — the tank gates input on frozen_ == true, and
        //     the ER bank must do the same)
        //   - softOnset ramp and limiter see the combined ER+tank signal,
        //     so onset smoothing and peak reduction apply uniformly instead
        //     of letting ER spikes slip past unsoftened / unlimited.
        if (erEntry_ != nullptr && erMask_ > 0)
        {
            // When frozen_, write silence so the delay line drains out
            // with no new content, but still READ from it so the already-
            // buffered tail decays out naturally.
            const float xL = frozen_ ? 0.0f : inputL[i];
            const float xR = frozen_ ? 0.0f : inputR[i];
            erBufL_[static_cast<size_t> (erWritePos_)] = xL;
            erBufR_[static_cast<size_t> (erWritePos_)] = xR;
            float erL = 0.0f;
            float erR = 0.0f;
            const dusk::baked::ErTap* taps = erEntry_->taps;
            const int n = erEntry_->tapCount;
            for (int t = 0; t < n; ++t)
            {
                int rp = (erWritePos_ - erTapDelays_[t]) & erMask_;
                erL += erBufL_[static_cast<size_t> (rp)] * taps[t].gainL;
                erR += erBufR_[static_cast<size_t> (rp)] * taps[t].gainR;
            }
            erWritePos_ = (erWritePos_ + 1) & erMask_;
            scaledL = scaledL * tankMix_ + erL * erMix_;
            scaledR = scaledR * tankMix_ + erR * erMix_;
        }

        // Soft output onset ramp: smooths the initial transient spike from
        // early taps (now including ER bank peaks).
        ...
        // Peak limiter: fast-attack / slow-release envelope follower with gain
        // reduction. Sees the combined ER+tank signal so ER peaks get limited
        // too (previously the limiter only saw the tank signal).
        if (limiterThreshold_ > 0.0f)
        {
            ...
            if (peakLR > limiterEnv_)
                ...
                limiterEnv_ = limiterReleaseCoeff_ * limiterEnv_
                            + (1.0f - limiterReleaseCoeff_) * peakLR;
...
float VoxPlatePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void VoxPlatePresetEngine::setDecayTime (float seconds)
{
...
void VoxPlatePresetEngine::setBassMultiply (float mult)
{
...
void VoxPlatePresetEngine::setTrebleMultiply (float mult)
{
...
void VoxPlatePresetEngine::setCrossoverFreq (float hz)
{
...
void VoxPlatePresetEngine::setHighCrossoverFreq (float hz)
{
...
void VoxPlatePresetEngine::setAirDampingScale (float scale)
{
...
void VoxPlatePresetEngine::setModDepth (float depth)
{
...
void VoxPlatePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void VoxPlatePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
...
void VoxPlatePresetEngine::setSize (float size)
{
...
void VoxPlatePresetEngine::setSoftOnsetMs (float ms)
{
...
void VoxPlatePresetEngine::setDelayScale (float scale)
{
...
void VoxPlatePresetEngine::setNoiseModDepth (float samples)
{
...
void VoxPlatePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void VoxPlatePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void VoxPlatePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void VoxPlatePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void VoxPlatePresetEngine::setSizeRange (float min, float max)
{
...
void VoxPlatePresetEngine::setDecayBoost (float boost)
{
...
void VoxPlatePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void VoxPlatePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void VoxPlatePresetEngine::clearBuffers()
{
...
void VoxPlatePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void VoxPlatePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void VoxPlatePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class VoxPlatePresetWrapper : public PresetEngineBase
{
public:
    VoxPlatePresetWrapper() = default;
    ~VoxPlatePresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Vox Plate"; }
...
{
    return std::unique_ptr<PresetEngineBase> (new VoxPlatePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gVoxPlatePresetRegistrar ("PresetVoxPlate", &createVoxPlatePreset);
