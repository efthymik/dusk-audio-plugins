...
//
// Per-preset reverb engine for "Rich Chamber".
...
// any other preset. The class has been renamed to RichChamberPresetEngine to avoid
// ODR conflicts.

#include "RichChamberPreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetRichChamber in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.436506f;
    constexpr float kBakedSizeRangeMax       = 1.309518f;
    ...
    constexpr float kVvAirDampingScale   = 0.829046f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.873012f — size range now absolute
    constexpr float kVvTiltLowDb         = 0.939866f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -2.32922f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.900456f;
    ...
    // Slightly lengthened from 0.69502f to match VV tail without overshoot.
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 1.2f
    ...
    // Rebalanced — original had +12 boost at 100Hz AND mid cuts that hollowed
    // DV once ER was reduced. Two-pass tune: neutralize cuts, then fill bass
    // gap (-7dB at 80-125) and tame midrange (+6dB at 315-800).
    constexpr float kCorrEqDb[kCorrEqBandCount] = { 2.0f, 2.57f, -0.06f, 0.0f, 0.0f, 3.0f, 5.0f, 5.0f, 5.0f, 4.0f, -1.49f, 1.0f };
    ...
    constexpr float kOnsetEnvelope[kOnsetTableSize] = {
        ...
        0.9237f, 0.3586f, 0.2180f, 0.1613f, 0.1330f, 0.1224f, 0.1400f, 0.1761f,
        0.1492f, 0.1448f, 0.1375f, 0.1232f, 0.1136f, 0.1218f, 0.1513f, 0.1540f,
        0.1797f, 0.2296f, 0.2492f, 0.2308f, 0.2191f, 0.2580f, 0.2969f, 0.3403f,
        0.3167f, 0.2928f, 0.2733f, 0.2749f, 0.2860f, 0.3101f, 0.3329f, 0.3254f,
        0.4018f, 0.4059f, 0.4281f, 0.3593f, 0.3497f, 0.3173f, 0.3361f, 0.3973f,
        0.4508f, 0.4751f, 0.4912f, 0.4639f, 0.5077f, 0.6638f, 0.6930f, 0.5383f,
        0.3339f, 0.3495f, 0.4113f, 0.5814f, 0.6277f, 0.8515f, 0.6770f, 1.0000f
    };
...
class RichChamberPresetEngine
{
public:
    RichChamberPresetEngine();
...
private:
    ...
    struct Tank
    {
        ...
        // Body-tail shaper state (see process()).
...
void RichChamberPresetEngine::DelayLine::allocate (int maxSamples)
{
...
void RichChamberPresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float RichChamberPresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
// -----------------------------------------------------------------------
void RichChamberPresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void RichChamberPresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
// -----------------------------------------------------------------------
RichChamberPresetEngine::RichChamberPresetEngine()
{
...
// -----------------------------------------------------------------------
void RichChamberPresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
// -----------------------------------------------------------------------
void RichChamberPresetEngine::process (const float* inputL, const float* inputR,
                        float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
// -----------------------------------------------------------------------
float RichChamberPresetEngine::readOutputTap (const OutputTap& tap) const
{
...
// -----------------------------------------------------------------------
void RichChamberPresetEngine::setDecayTime (float seconds)
{
    decayTime_ = std::max (seconds, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void RichChamberPresetEngine::setBassMultiply (float mult)
{
    bassMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void RichChamberPresetEngine::setTrebleMultiply (float mult)
{
    trebleMultiply_ = std::max (mult, 0.1f);
    if (prepared_) updateDecayCoefficients();
}

void RichChamberPresetEngine::setCrossoverFreq (float hz)
{
...
void RichChamberPresetEngine::setSize (float size)
{
...
void RichChamberPresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
        for (int t = 0; t < kNumTanks; ++t)
...
void RichChamberPresetEngine::setLateGainScale (float scale) { lateGainScale_ = std::max (scale, 0.0f); }

void RichChamberPresetEngine::setHighCrossoverFreq (float hz)
{
...
void RichChamberPresetEngine::setAirDampingScale (float scale)
{
...
void RichChamberPresetEngine::setSizeRange (float min, float max)
{
...
void RichChamberPresetEngine::setDecayBoost (float boost)
{
...
void RichChamberPresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void RichChamberPresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void RichChamberPresetEngine::clearBuffers()
{
    for (int t = 0; t < kNumTanks; ++t)
    {
        auto& tank = tanks_[t];
...
// -----------------------------------------------------------------------
void RichChamberPresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void RichChamberPresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
...
void RichChamberPresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class RichChamberPresetWrapper : public PresetEngineBase
{
public:
    RichChamberPresetWrapper() = default;
    ~RichChamberPresetWrapper() override = default;
    ...
    const char* getPresetName() const override { return "Rich Chamber"; }
...
private:
    RichChamberPresetEngine engine_;
...
{
    return std::unique_ptr<PresetEngineBase> (new RichChamberPresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gRichChamberPresetRegistrar ("PresetRichChamber", &createRichChamberPreset);
