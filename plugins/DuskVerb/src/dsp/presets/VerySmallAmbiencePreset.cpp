...
//
// Per-preset reverb engine for "Very Small Ambience".
...
// any other preset. The class has been renamed to VerySmallAmbiencePresetEngine to avoid
// ODR conflicts.

#include "VerySmallAmbiencePreset.h"
...
namespace {
    ...
    // Baked algorithm-config constants from kPresetVerySmallAmbience in AlgorithmConfig.h
    // at generation time. Editing these here has no effect on other presets.
    constexpr float kBakedLateGainScale      = 0.22f;
    constexpr float kBakedSizeRangeMin       = 0.243893f;
    constexpr float kBakedSizeRangeMax       = 0.731677f;
    constexpr float kBakedAirDampingScale    = 0.8f;
    constexpr float kBakedNoiseModDepth      = 20.0f;  // increased to smear resonant modes
    constexpr float kBakedTrebleMultScale    = 0.84f;
    constexpr float kBakedTrebleMultScaleMax = 1.5f;
    constexpr float kBakedBassMultScale      = 1.0f;  // un-baked from 0.87f — bass knob now shows real multiplier
    ...
    constexpr float kVvAirDampingScale   = 0.97715f;
    constexpr float        kVvDelayScale        = 1.0f;  // un-baked from 0.487785f — size range now absolute
    constexpr float kVvTiltLowDb         = -0.512584f;
    constexpr float kVvTiltLowHz         = 400.0f;
    constexpr float kVvTiltHighDb        = -4.0142f;
    constexpr float kVvTiltHighHz        = 5000.0f;
    constexpr float kVvStereoWidth       = 0.923505f;
    ...
    constexpr float kVvDecayTimeScale    = 1.0f;   // un-baked from 0.66069f
    ...
    constexpr float kCorrEqDb[kCorrEqBandCount] = { -23.21f, -24.0f, -24.0f, -24.0f, -24.0f, -24.0f, -24.0f, -21.01f, 5.08427f, -5.81f, -23.72f, 24.0f };
    ...
    constexpr float kAmDepth = 0.120f;
...
class VerySmallAmbiencePresetEngine
{
public:
    VerySmallAmbiencePresetEngine();
...
private:
    ...
    float limiterReleaseMs_ = 50.0f;      // Cached release time for sample-rate recomputation
...
void VerySmallAmbiencePresetEngine::DelayLine::allocate (int maxSamples)
{
...
void VerySmallAmbiencePresetEngine::DelayLine::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}

float VerySmallAmbiencePresetEngine::DelayLine::readInterpolated (float delaySamples) const
{
...
void VerySmallAmbiencePresetEngine::Allpass::allocate (int maxSamples)
{
    int bufSize = DspUtils::nextPowerOf2 (maxSamples + 1);
    buffer.assign (static_cast<size_t> (bufSize), 0.0f);
    mask = bufSize - 1;
...
void VerySmallAmbiencePresetEngine::Allpass::clear()
{
    std::fill (buffer.begin(), buffer.end(), 0.0f);
    writePos = 0;
}
...
VerySmallAmbiencePresetEngine::VerySmallAmbiencePresetEngine()
{
...
void VerySmallAmbiencePresetEngine::setHallScale (bool enable)
{
    if (enable)
    {
...
void VerySmallAmbiencePresetEngine::prepare (double sampleRate, int /*maxBlockSize*/)
{
...
void VerySmallAmbiencePresetEngine::process (const float* inputL, const float* inputR,
                            float* outputL, float* outputR, int numSamples)
{
    if (! prepared_)
        return;
...
float VerySmallAmbiencePresetEngine::readOutputTap (const OutputTap& tap) const
{
...
void VerySmallAmbiencePresetEngine::setDecayTime (float seconds)
{
...
void VerySmallAmbiencePresetEngine::setBassMultiply (float mult)
{
...
void VerySmallAmbiencePresetEngine::setTrebleMultiply (float mult)
{
...
void VerySmallAmbiencePresetEngine::setCrossoverFreq (float hz)
{
...
void VerySmallAmbiencePresetEngine::setHighCrossoverFreq (float hz)
{
...
void VerySmallAmbiencePresetEngine::setAirDampingScale (float scale)
{
...
void VerySmallAmbiencePresetEngine::setModDepth (float depth)
{
...
void VerySmallAmbiencePresetEngine::setModRate (float hz)
{
    modRateHz_ = hz;
    if (prepared_)
        updateLFORates();
...
void VerySmallAmbiencePresetEngine::setLimiter (float thresholdDb, float releaseMs)
{
    if (thresholdDb >= 0.0f)
    {
        limiterThreshold_ = 0.0f;  // Disabled
        limiterReleaseMs_ = 0.0f;
        return;
    }
...
void VerySmallAmbiencePresetEngine::setSize (float size)
{
...
void VerySmallAmbiencePresetEngine::setSoftOnsetMs (float ms)
{
...
void VerySmallAmbiencePresetEngine::setDelayScale (float scale)
{
...
void VerySmallAmbiencePresetEngine::setNoiseModDepth (float samples)
{
...
void VerySmallAmbiencePresetEngine::setOutputTaps (const OutputTap* left, const OutputTap* right)
{
    if (left && right)
    {
...
void VerySmallAmbiencePresetEngine::applyTapGains (const float* leftGains, const float* rightGains)
{
...
void VerySmallAmbiencePresetEngine::setFreeze (bool frozen)
{
    frozen_ = frozen;
    if (frozen)
    {
...
void VerySmallAmbiencePresetEngine::setLateGainScale (float scale)
{
    lateGainScale_ = std::max (scale, 0.0f);
}

void VerySmallAmbiencePresetEngine::setSizeRange (float min, float max)
{
...
void VerySmallAmbiencePresetEngine::setDecayBoost (float boost)
{
...
void VerySmallAmbiencePresetEngine::setStructuralHFDamping (float hz)
{
    if (hz <= 0.0f)
    {
        structHFCoeff_ = 0.0f;
...
void VerySmallAmbiencePresetEngine::setTerminalDecay (float thresholdDB, float factor)
{
...
void VerySmallAmbiencePresetEngine::clearBuffers()
{
    auto clearTank = [] (Tank& tank, uint32_t lfoSeed, uint32_t noiseSeed)
    {
        ...
        tank.lfoPhase = 0.0f;
        tank.lfoPRNG = lfoSeed;
        tank.noiseState = noiseSeed;
    };
...
void VerySmallAmbiencePresetEngine::updateDelayLengths()
{
    float rateRatio = static_cast<float> (sampleRate_ / kBaseSampleRate);
...
void VerySmallAmbiencePresetEngine::updateDecayCoefficients()
{
    float sr = static_cast<float> (sampleRate_);
    ...
    auto updateTankDamping = [&] (Tank& tank)
    {
        ...
        tank.damping.setCrossovers (fbCoeffs);
    };
...
void VerySmallAmbiencePresetEngine::updateLFORates()
{
    float sr = static_cast<float> (sampleRate_);
...
class VerySmallAmbiencePresetWrapper : public PresetEngineBase
{
public:
    VerySmallAmbiencePresetWrapper() = default;
    ~VerySmallAmbiencePresetWrapper() override = default;
    ...
    void prepare (double sampleRate, int maxBlockSize) override
    {
        ...
        if (lastStructHFHz_ > 0.0f)
            engine_.setStructuralHFDamping (lastStructHFHz_);
        ...
        gatePeakRMS_ = 0.0f;
        gateGainEnv_ = 1.0f;
        // Sample-rate-independent gate coefficients.
        // Time constants derived from the original 48kHz hardcoded values.
        {
            constexpr float kGateRmsTauMs   = 41.65f;   // 0.9995 @ 48kHz
            constexpr float kGatePeakTauMs  = 2083.0f;  // 0.99999 @ 48kHz
            constexpr float kGateDecayTauMs = 173.6f;   // 0.99988 @ 48kHz
            constexpr float kGateAttackMs   = 20.83f;   // 0.001 @ 48kHz
            float sr = static_cast<float> (sampleRate);
            gateRMSAlpha_       = std::exp (-1000.0f / (kGateRmsTauMs * sr));
            gateRMSBeta_        = 1.0f - gateRMSAlpha_;
            gatePeakDecay_      = std::exp (-1000.0f / (kGatePeakTauMs * sr));
            gateDecayPerSample_ = std::exp (-1000.0f / (kGateDecayTauMs * sr));
            gateAttackInc_      = 1.0f / (kGateAttackMs * 0.001f * sr);
        }
                  ...
                  float* outputL, float* outputR, int numSamples) override
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            constexpr float kWrapperSafetyClip = 4.0f;  // Must match engine kSafetyClip
            outputL[i] = std::clamp (mid + side * kStereoWidth, -kWrapperSafetyClip, kWrapperSafetyClip);
            outputR[i] = std::clamp (mid - side * kStereoWidth, -kWrapperSafetyClip, kWrapperSafetyClip);
        }
        // PATCH_POINT_POST_ENGINE

        // --- Output-side tail gate (fixes tail_slope_ratio) ---
        // VV cuts tail at -79dB/s, DV natural is -25dB/s. Compounding gain
        // envelope decays once tail drops below threshold.
        if (! frozen_)
        {
            constexpr float kGateLinearThresh = 10.0f;    // -10dB below peak
            for (int i = 0; i < numSamples; ++i)
            {
                float e = 0.5f * (outputL[i] * outputL[i] + outputR[i] * outputR[i]);
                gateRMS_ = gateRMSAlpha_ * gateRMS_ + gateRMSBeta_ * e;
                if (gateRMS_ > gatePeakRMS_) gatePeakRMS_ = gateRMS_;
                else gatePeakRMS_ *= gatePeakDecay_;
                float ratio = gatePeakRMS_ / std::max (gateRMS_, 1e-20f);
                if (ratio > kGateLinearThresh && gatePeakRMS_ > 1e-12f)
                    gateGainEnv_ *= gateDecayPerSample_;
                else
                    gateGainEnv_ = std::min (gateGainEnv_ + gateAttackInc_, 1.0f);
    ...
    void setFreeze (bool frozen) override
    {
        ...
        if (wasTransition)
        {
            // Reset envelope/AM state so unfreezing doesn't use stale history
            gateRMS_ = 0.0f;
            gatePeakRMS_ = 0.0f;
            gateGainEnv_ = 1.0f;
            amPhase_ = 0.0f;
    ...
    void setAirDampingScale (float scale) override
    {
        ...
        engine_.setAirDampingScale (scale * kBakedAirDampingScale * kVvAirDampingScale);
    }
    ...
    const char* getPresetName() const override { return "Very Small Ambience"; }
...
private:
    VerySmallAmbiencePresetEngine engine_;
    ...
    // Sample-rate-independent gate coefficients (computed in prepare())
    float gateRMSAlpha_ = 0.9995f;
    float gateRMSBeta_ = 0.0005f;
    float gatePeakDecay_ = 0.99999f;
    float gateDecayPerSample_ = 0.99988f;
    float gateAttackInc_ = 0.001f;
    // PATCH_POINT_MEMBERS
};
...
{
    return std::unique_ptr<PresetEngineBase> (new VerySmallAmbiencePresetWrapper());
}

// Self-register at static init time so DuskVerbEngine can look up this preset by name
static PresetEngineRegistrar gVerySmallAmbiencePresetRegistrar ("PresetVerySmallAmbience", &createVerySmallAmbiencePreset);
