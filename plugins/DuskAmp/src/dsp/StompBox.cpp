...
void StompBox::updateMidHump()
{
    ...
    midHump_.b0 = alpha * invA0;   // BPF (constant 0dB peak gain, Audio EQ Cookbook)
    midHump_.b1 = 0.0f;
    midHump_.b2 = -alpha * invA0;
...
void StompBox::updateToneCoeff()
{
    ...
    // Bilinear transform: g = tan(pi * fc / fs), coeff = g / (1 + g)
    float g = std::tan (kPi * cutoff / static_cast<float> (sampleRate_));
    toneLPCoeff_ = g / (1.0f + g);
}
