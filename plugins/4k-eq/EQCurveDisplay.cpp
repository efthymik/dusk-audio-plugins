...
EQCurveDisplay::EQCurveDisplay(FourKEQ& processor)
...
void EQCurveDisplay::timerCallback()
{
    ...
    if (auto* p = params.getRawParameterValue("lf_bell"))
        newParams.lfBell = p->load() > 0.5f;
    if (auto* p = params.getRawParameterValue("lm_gain"))
        newParams.lmGain = p->load();
    if (auto* p = params.getRawParameterValue("lm_freq"))
        newParams.lmFreq = p->load();
    if (auto* p = params.getRawParameterValue("lm_q"))
        newParams.lmQ = p->load();
    if (auto* p = params.getRawParameterValue("hm_gain"))
        newParams.hmGain = p->load();
    if (auto* p = params.getRawParameterValue("hm_freq"))
        newParams.hmFreq = p->load();
    if (auto* p = params.getRawParameterValue("hm_q"))
        newParams.hmQ = p->load();
    if (auto* p = params.getRawParameterValue("hf_gain"))
        newParams.hfGain = p->load();
    if (auto* p = params.getRawParameterValue("hf_freq"))
        newParams.hfFreq = p->load();
    if (auto* p = params.getRawParameterValue("hf_bell"))
        newParams.hfBell = p->load() > 0.5f;
    if (auto* p = params.getRawParameterValue("eq_type"))
...
float EQCurveDisplay::freqToX(float freq, const juce::Rectangle<float>& area) const
{
...
float EQCurveDisplay::xToFreq(float x, const juce::Rectangle<float>& area) const
{
    float logMin = std::log10(minFreq);
    float logMax = std::log10(maxFreq);
...
void EQCurveDisplay::drawGrid(juce::Graphics& g, const juce::Rectangle<float>& area)
{
...
void EQCurveDisplay::drawCombinedCurve(juce::Graphics& g, const juce::Rectangle<float>& area)
{
    juce::Path path;
    juce::Path fillPath;
    bool pathStarted = false;
...
float EQCurveDisplay::calculateHPFResponse(float freq) const
{
    ...
    // Use proper 3rd order magnitude response: |H(jw)| = (w/wc)^3 / sqrt(1 + (w/wc)^6)
    // Simplified to asymptotic: below cutoff = 18dB/oct rolloff, above = 0dB
    float response;
    if (ratio < 1.0f)
    {
...
float EQCurveDisplay::calculateLFResponse(float freq) const
{
    ...
    else
    {
        ...
        // Transition range is about 2 octaves (logRatio from -0.6 to +0.6)
...
float EQCurveDisplay::calculateLMFResponse(float freq) const
{
...
float EQCurveDisplay::calculateHMFResponse(float freq) const
{
...
float EQCurveDisplay::calculateHFResponse(float freq) const
{
