...
public:
    ...
    float processSample(float input, float drive, bool isLeftChannel)
    {
        ...
        // Uses Marinair-style (E-Series) or Carnhill-style (G-Series) transformer modeling
        ...
        // Add console noise floor (-90dB RMS, typical for analog consoles)
...
private:
    ...
    float processInputTransformer(float input, float drive)
    {
        ...
        // Drive range allows authentic "pushed" sound without excessive aliasing
        ...
        if (abs_x < 0.9f)
        {
            // Linear region - no saturation (console operates here at -18dB)
            saturated = driven;
        }
        else if (abs_x < 1.5f)
        {
    ...
    float processOpAmpStage(float input, float drive)
    {
        ...
        // Positive half-cycle (toward V+ rail, ~+15V in console)
        if (driven > 0.0f)
        {
            if (driven < 1.0f)
            {
                // Linear region - console operates here at -18dB
        ...
        // Negative half-cycle (toward V- rail, ~-15V in console)
        else
        {
            if (driven > -1.0f)
            {
        ...
        // Console-specific harmonic shaping - op-amps are very linear until driven hard
        //
        // DESIGN DECISION: Threshold difference dominates harmonic behavior
        // E-Series (0.6 threshold): Clean at low drive, strong harmonics when engaged
        // G-Series (0.05 threshold): Subtle harmonics across entire drive range
