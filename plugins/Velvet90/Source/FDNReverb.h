...
enum class ColorMode
{
    ...
    Eighties,       // 1980s: classic 80s era — cleaner but still colored
...
// Gentler slope than biquad — matches vintage-style progressive HF darkening
class DampingFilter
{
public:
    void prepare(double sr)
...
//==============================================================================
// Complex LFO system (vintage-style with multiple rates + random)
...
public:
    ...
    float process()
    {
        // Three sine LFOs at different rates (classic 80s-style)
...
//==============================================================================
// Mode-specific parameters (enhanced for vintage-style sound)
...
inline ModeParameters getPlateParameters()
{
    return {
        ...
        0.92f,          // highDecayMult: vintage-calibrated
...
inline ModeParameters getRoomParameters()
{
    return {
        ...
        1.03f,          // lowDecayMult: slight bass warmth
        1.0f,           // midDecayMult: neutral mid
        0.90f,          // highDecayMult: vintage-calibrated
        4000.0f,        // highFreq (band 2/3 boundary)
...
inline ModeParameters getHallParameters()
{
    return {
        ...
        1.03f,          // lowDecayMult: slight bass warmth
        1.0f,           // midDecayMult: neutral mid
        0.88f,          // highDecayMult: vintage-calibrated (hall = darker)
...
inline ModeParameters getChorusSpaceParameters()
{
    return {
        ...
        1.03f,          // lowDecayMult: slight warmth
        1.0f,           // midDecayMult: neutral mid
        0.88f,          // highDecayMult: vintage-calibrated
        4000.0f,        // highFreq (band 2/3 boundary)
...
public:
    ...
    {
        sampleRate = sr;
        delayLineL.prepare(sr, 520.0f);  // 500ms + margin
        delayLineR.prepare(sr, 520.0f);
        clear();
    }
    ...
    void setDelayMs(float ms)
    {
        ...
        delaySamples = static_cast<int>(clampedMs * sampleRate / 1000.0);
        if (delaySamples > 0)
        {
            delayLineL.setDelayMs(clampedMs);
            delayLineR.setDelayMs(clampedMs);
        }
    }

    void setFeedback(float fb) { feedback = std::clamp(fb, 0.0f, 0.9f); }
    void setPingPong(float pp) { pingPong = std::clamp(pp, 0.0f, 1.0f); }
    ...
    void process(float& wetL, float& wetR)
    {
        ...
        // Blend between straight feedback (mono) and crossed feedback (ping-pong)
        float straightFbL = fbStateL * feedback;
        float straightFbR = fbStateR * feedback;
        float crossedFbL  = fbStateR * feedback;
        float crossedFbR  = fbStateL * feedback;

        float blendedFbL = straightFbL * (1.0f - pingPong) + crossedFbL * pingPong;
        float blendedFbR = straightFbR * (1.0f - pingPong) + crossedFbR * pingPong;

        float echoInputL = wetL + blendedFbL;
        float echoInputR = wetR + blendedFbR;
        float echoL = delayLineL.process(echoInputL);
        float echoR = delayLineR.process(echoInputR);
        fbStateL = echoL;
        fbStateR = echoR;
...
//==============================================================================
// Input-triggered envelope shaper for non-linear reverb effects
// Detects input transients and shapes the reverb tail accordingly
...
public:
    ...
    // Feed input level for transient detection (call before process)
    void feedInput(float inputL, float inputR)
    {
        float level = std::max(std::abs(inputL), std::abs(inputR));
        // Smooth envelope follower (fast attack, slow release)
        float attackCoeff = 0.01f;  // ~0.2ms attack
        float releaseCoeff = 0.0001f;  // ~100ms release
        if (level > inputEnvelope)
            inputEnvelope += attackCoeff * (level - inputEnvelope);
        else
            inputEnvelope += releaseCoeff * (level - inputEnvelope);

        // Trigger on transient (rising above threshold)
        constexpr float triggerThreshold = 0.001f;  // -60dB
        constexpr float retriggerThreshold = 0.0003f;  // hysteresis
        if (!triggered && inputEnvelope > triggerThreshold)
        {
            triggered = true;
            sampleCounter = 0;  // Reset envelope
        }
        else if (triggered && inputEnvelope < retriggerThreshold)
        {
            // Allow re-triggering once input drops below hysteresis
            triggered = false;
        }
    }
    ...
    void process(float& wetL, float& wetR)
    {
        ...
        // Shaped signal
        float shapedL = wetL * env;
        float shapedR = wetR * env;
...
//==============================================================================
// Sidechain dynamics processor — input-level-driven gain modulation on wet signal
// Positive amount = expansion (louder input → louder reverb)
// Negative amount = compression/ducking (louder input → quieter reverb)
...
public:
    ...
    void setAmount(float a) { amount = std::clamp(a, -1.0f, 1.0f); }
    ...
    void process(float& wetL, float& wetR)
    {
        ...
        // pow(envelope, amount) naturally handles both directions:
        // amount > 0: quiet input (env < 1) → gain < 1 (expansion/follow)
        // amount < 0: quiet input (env < 1) → gain > 1 (compression/duck)
        float env = std::max(inputEnvelope, 1e-6f);
        float gain = std::pow(env, amount);
        gain = std::clamp(gain, 0.01f, 10.0f);
...
private:
    void updateCoeffs()
    {
        // speed 0.0 = slow (100ms attack, 1000ms release)
        // speed 1.0 = fast (1ms attack, 50ms release)
        float attackMs  = 100.0f - speed * 99.0f;
        float releaseMs = 1000.0f - speed * 950.0f;
        attackCoeff  = 1.0f - std::exp(-1.0f / (static_cast<float>(sampleRate) * attackMs / 1000.0f));
        releaseCoeff = 1.0f - std::exp(-1.0f / (static_cast<float>(sampleRate) * releaseMs / 1000.0f));
    }
...
//==============================================================================
// Main FDN Reverb Engine (vintage-enhanced with professional upgrades)
class FDNReverb
{
public:
    ...
    void prepare(double sr, int /*maxBlockSize*/)
    {
        ...
        // Prepare envelope shaper, echo delay, and dynamics processor
        envelopeShaper.prepare(sampleRate);
        echoDelay.prepare(sampleRate);
    ...
    void setColor(ColorMode color)
    {
        ...
        switch (color)
        {
            ...
            {
                // 1980s: classic 80s era — cleaner but still colored
    ...
    // Stereo polarity inversion: controls how much the R channel output taps
    // use inverted polarity relative to L. At 0 = normal (correlated), at 1 = fully
    // anti-correlated (opposite polarity on alternating delay outputs for R channel).
    // This enables matching vintage presets with negative stereo correlation.
    void setStereoInvert(float inv)
    {
        stereoInvert = std::clamp(inv, 0.0f, 1.0f);
    }

    // Resonance: reduces diffusion and shifts delay ratios toward integer multiples,
    // creating metallic/resonant coloration. 0 = smooth (default), 1 = fully metallic.
    void setResonance(float res)
    {
        resonance = std::clamp(res, 0.0f, 1.0f);
        updateTankDiffusion();  // Reduces diffuser feedback
        updateDelayTimes();     // Shifts toward resonant ratios
    }
    ...
    // Echo delay setters (decoupled from envelope — works independently)
    void setEchoDelay(float ms) { echoDelay.setDelayMs(ms); }
    void setEchoFeedback(float fb) { echoDelay.setFeedback(fb); }
    void setEchoPingPong(float pp) { echoDelay.setPingPong(pp); }
    void setDynAmount(float a) { dynamicsProcessor.setAmount(a); }
    void setDynSpeed(float s) { dynamicsProcessor.setSpeed(s); }
    ...
    void process(float inputL, float inputR, float& outputL, float& outputR)
    {
        ...
        // Sum delay outputs (late reverb) with optional stereo anti-correlation
        // When stereoInvert > 0, alternating delay line outputs are sign-inverted
        // for the R channel, producing anti-correlated stereo (negative correlation)
        float lateL = 0.0f, lateR = 0.0f;
        for (int i = 0; i < NUM_DELAYS; ++i)
        {
            size_t idx = static_cast<size_t>(i);
            lateL += delayOutputsL[idx];
            // Alternate sign for R: even indices positive, odd indices inverted
            float rSign = (i % 2 == 0) ? 1.0f : (1.0f - 2.0f * stereoInvert);
            lateR += delayOutputsR[idx] * rSign;
        }
        ...
        // Echo delay (independent of envelope — always available)
        echoDelay.process(wetL, wetR);

        // Envelope shaper (input-triggered for non-linear presets: gate, reverse, swell)
        envelopeShaper.feedInput(inputL, inputR);
        envelopeShaper.process(wetL, wetR);

        // Sidechain dynamics (input-level-driven gain modulation on wet signal)
        dynamicsProcessor.feedInput(inputL, inputR);
        dynamicsProcessor.process(wetL, wetR);
...
private:
    ...
    float stereoInvert = 0.0f;  // 0 = normal, 1 = anti-correlated R channel
    float resonance = 0.0f;    // 0 = smooth, 1 = metallic/resonant
    ...
    // Echo delay (decoupled from envelope — works independently)
    EchoDelay echoDelay;

    // Sidechain dynamics (input-level-driven gain modulation)
    ...
    void updateDelayTimes()
    {
        ...
        // Integer-ratio delay times (for metallic/resonant character)
        // These are simple multiples of a base delay, creating strong comb filtering
        constexpr std::array<float, NUM_DELAYS> integerRatioTimes = {
        ...
        for (int i = 0; i < NUM_DELAYS; ++i)
        {
            ...
            // Blend between prime-based (smooth) and integer-ratio (resonant) delay times
            float primeDelay = modeParams.delayTimesMs[idx];
            float integerDelay = integerRatioTimes[idx];
            float blendedDelay = primeDelay * (1.0f - resonance) + integerDelay * resonance;

            baseDelayTimesL[idx] = blendedDelay * roomScale;
            baseDelayTimesR[idx] = blendedDelay * stereoOffsets[idx] * roomScale;
    ...
    {
        // Auto-scale bass multiplier down for short reverbs (< 0.5s target decay)
        // This prevents bass RT60 from being 2-4x longer than target in tight ambiences
        float bassAutoScale = 1.0f;
        if (targetDecay < 0.5f)
        {
            // Linear ramp: at 0.5s => scale 1.0, at 0.1s => scale 0.3
            bassAutoScale = 0.3f + (targetDecay / 0.5f) * 0.7f;
        }

        float lowMult = modeParams.lowDecayMult * userBassMult * bassAutoScale;
    ...
    void updateDiffusion()
    {
        ...
        for (int i = 0; i < NUM_INPUT_DIFFUSERS; ++i)
        {
            // Alternate feedback sign to reduce metallic coloration (vintage digital technique)
    ...
    void updateTankDiffusion()
    {
        ...
        // Resonance reduces diffuser feedback — at resonance=1.0, feedback drops
        // to near zero, letting comb-filter peaks through (metallic character)
        float diffReduction = 1.0f - resonance * 0.9f;  // 1.0 → 0.1

        for (int i = 0; i < NUM_TANK_DIFFUSERS; ++i)
        {
            // Alternate feedback sign for tank diffusers too
            float sign = (i % 2 == 0) ? 1.0f : -1.0f;
            float fb = lateDiffusion * 0.6f * sign * diffReduction;
