#!/usr/bin/env python3
"""
Batch Optimizer — Process all PCM 90 IRs through Velvet 90 parameter optimizer.

Reads the IR-to-preset mapping, runs the optimizer for each IR with its
assigned Velvet 90 mode, and saves individual JSON results that can be
...
Usage:
    ...
    # Use 8 parallel workers with warm-start from previous results
    python batch_optimize_all.py -j 8 --warm-start results_macos -o results_v7
...
def optimize_single(args_tuple):
    ...
    module-level _plugin_cache in velvet90_capture.py.

    Args:
        args_tuple: (mapping_dict, output_dir, max_iterations, resume, index, total, warm_start_dir)

    Returns:
        Result dict or None if skipped.
    """
    m, output_dir, max_iterations, resume, index, total, warm_start_dir = args_tuple
    ...
    if warm_start_dir:
        ws_file = os.path.join(warm_start_dir, f"{safe_name}.json")
    ...
    ws_label = " [warm]" if warm_start_params else " [cold]"
    print(f"\n[{worker}] [{index}/{total}] {preset_name} "
          f"({m['mode_name']}, {m['category']}){ws_label}", flush=True)
...
def run_batch(output_dir='results', max_iterations=80, resume=True,
    ...
    Args:
        ...
        category_filter: Only process this Velvet 90 category
        preset_filter: Only process this preset name (partial match)
        warm_start_dir: Directory with previous results to warm-start from
    """
    ...
    print(f"{'='*60}")
    print(f"  Velvet 90 Batch Optimizer — PCM 90 IR Matching")
    print(f"{'='*60}")
    ...
    if warm_start_dir:
        print(f"  Warm-start: {os.path.abspath(warm_start_dir)}")
    print(f"{'='*60}\n", flush=True)
    ...
    work_items = [
        (m, output_dir, max_iterations, resume, i + 1, total, warm_start_dir)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Batch optimize all PCM 90 IRs for Velvet 90',
    ...
    parser.add_argument('--warm-start', default=None,
                        help='Directory with previous results to warm-start from')
