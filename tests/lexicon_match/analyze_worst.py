...
For each preset:
  - Loads the target Lexicon IR
  - Renders SilkVerb IR using the optimized parameters
...
def format_band_rt60_table(target_bands: dict, silkverb_bands: dict, diffs: dict) -> str:
    """Format a side-by-side band RT60 comparison table."""
    all_bands = sorted(
        set(list(target_bands.keys()) + list(silkverb_bands.keys())),
        key=lambda b: int(b.replace('Hz', ''))
    )
    lines = ["      Band      Target    SilkVerb    Diff"]
    lines.append("      " + "-" * 42)
    for band in all_bands:
        t_val = target_bands.get(band, 0.0)
        s_val = silkverb_bands.get(band, 0.0)
...
def main():
    ...
    for i, result in enumerate(worst):
        ...
        # Reconstruct SilkVerb params and render IR
        try:
            sv_params = SilkVerbParams.from_dict(params_dict)
        except Exception as e:
            print(f"  ERROR creating SilkVerb params: {e}")
            continue

        print(f"\n  SilkVerb params: mode={sv_params.mode}, color={sv_params.color}, "
        ...
        try:
            ...
            print(f"  ERROR capturing SilkVerb IR: {e}")
            import traceback
            traceback.print_exc()
            continue

        sv_profile = analyze_ir(sv_ir, target_sr, name=f"SilkVerb ({name})")
