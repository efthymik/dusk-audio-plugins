"""
SilkVerb IR Capture — Generate impulse responses from SilkVerb via pedalboard.

Renders a dirac impulse through SilkVerb with configurable parameters,
...
@dataclass
class SilkVerbParams:
    """Complete SilkVerb parameter set for IR capture."""
    mode: str = 'Hall'
    color: str = 'Now'
    size: str = '2.0s'
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'high_cut_hz': self.high_cut_hz, 'freeze': self.freeze,
        }

    @staticmethod
    def from_dict(d: dict) -> 'SilkVerbParams':
        return SilkVerbParams(**{k: v for k, v in d.items() if k != 'freeze' or isinstance(v, bool)})
...
def load_plugin(vst_path: str = '/home/marc/.vst3/SilkVerb.vst3'):
    """Load or return cached SilkVerb plugin instance."""
...
def capture_ir(params: SilkVerbParams,
               sr: int = 48000,
               duration_s: float = 6.0,
               vst_path: str = '/home/marc/.vst3/SilkVerb.vst3',
               normalize: bool = True) -> np.ndarray:
    """
    Capture an impulse response from SilkVerb.
...
def capture_ir_averaged(params: SilkVerbParams,
                        sr: int = 48000,
                        duration_s: float = 6.0,
                        n_averages: int = 1,
                        vst_path: str = '/home/marc/.vst3/SilkVerb.vst3') -> np.ndarray:
    """
...
def default_params_for_category(category: str) -> SilkVerbParams:
    """Get sensible default SilkVerb parameters for a Lexicon PCM 90 category."""
    if category == 'Halls':
