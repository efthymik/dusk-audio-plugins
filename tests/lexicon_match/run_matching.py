#!/usr/bin/env python3
"""
Lexicon PCM 90 / SilkVerb Matching Tool

Main entry point for comparing SilkVerb against Lexicon PCM 90 impulse responses
and optimizing parameters to match.

Usage:
    # Analyze a single Lexicon IR
    python run_matching.py analyze "path/to/ir.wav"

    # Compare SilkVerb (default params) against a Lexicon IR
    python run_matching.py compare "path/to/ir.wav" --mode Hall --color 1980s

    # Optimize SilkVerb params to match a Lexicon IR
...
from silkverb_capture import (
    SilkVerbParams, capture_ir, default_params_for_category, VALID_MODES, VALID_COLORS,
)
from parameter_optimizer import optimize_for_target, batch_optimize

LEXICON_BASE = '/mnt/Library/Library/Software/IMPULSE RESPONSES/Lexicon PCM 90 Impulse Set'
...
def get_representative_irs() -> list[tuple[str, str, str]]:
    ...
    for category, files in picks.items():
        for f in files:
            path = os.path.join(LEXICON_BASE, category, f)
...
def cmd_compare(args):
    """Compare SilkVerb output against a target IR."""
    # Load target
    ...
    silk_ir = capture_ir(params, sr=sr, duration_s=duration, normalize=True)
    silk_profile = analyze_ir(silk_ir, sr, name=f"SilkVerb({args.mode}/{args.color})")

    # Compare
    print(profile_summary(target_profile))
    print()
    print(profile_summary(silk_profile))
    print()

    result = compare_profiles(target_profile, silk_profile)
    print(result.summary())


def cmd_optimize(args):
    """Optimize SilkVerb parameters to match a target IR."""
...
def cmd_benchmark(args):
    """Run benchmark across representative Lexicon IRs."""
...
def cmd_plot(args):
    ...
    # Capture SilkVerb
    params = SilkVerbParams(mode=args.mode, color=args.color, size=args.size, mix=100.0,
                            room_size=args.room_size, damping=args.damping, pre_delay_ms=args.pre_delay)
    duration = min(target_profile.duration_s + 1.0, 8.0)
    silk_ir = capture_ir(params, sr=sr, duration_s=duration, normalize=True)
    silk_profile = analyze_ir(silk_ir, sr, name=f"SilkVerb({args.mode})")

    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle(f'IR Comparison: {target_name} vs SilkVerb({args.mode}/{args.color})', fontsize=14)

    # 1. Waveform comparison
    ax = axes[0, 0]
    t_target = np.arange(len(target_data[0])) / sr
    t_silk = np.arange(silk_ir.shape[1]) / sr
    ax.plot(t_target, target_data[0], alpha=0.7, label='Lexicon', linewidth=0.5)
    ax.plot(t_silk, silk_ir[0], alpha=0.7, label='SilkVerb', linewidth=0.5)
    ...
    ax = axes[0, 1]
    ax.plot(target_profile.edc_time, target_profile.edc, label='Lexicon', linewidth=1.5)
    ax.plot(silk_profile.edc_time, silk_profile.edc, label='SilkVerb', linewidth=1.5)
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Energy (dB)')
    ax.set_title(f'Energy Decay Curve (RT60: Lex={target_profile.rt60:.2f}s, SV={silk_profile.rt60:.2f}s)')
    ax.set_ylim(-70, 5)
    ax.legend()
    ...
    ax.plot(target_profile.spectral_centroid_time, target_profile.spectral_centroid_hz,
            label='Lexicon', linewidth=1.5)
    ax.plot(silk_profile.spectral_centroid_time, silk_profile.spectral_centroid_hz,
            label='SilkVerb', linewidth=1.5)
    ax.set_xlabel('Time (s)')
    ...
    if target_profile.freq_axis is not None:
        ...
        ax.semilogx(target_profile.freq_axis[mask], target_profile.freq_response_early[mask],
                     label='Lexicon', linewidth=1.5)
    if silk_profile.freq_axis is not None:
        mask = silk_profile.freq_axis > 0
        ax.semilogx(silk_profile.freq_axis[mask], silk_profile.freq_response_early[mask],
                     label='SilkVerb', linewidth=1.5)
    ...
    if silk_profile.freq_axis is not None:
        mask = silk_profile.freq_axis > 0
        ax.semilogx(silk_profile.freq_axis[mask], silk_profile.freq_response_late[mask],
                     label='SilkVerb', linewidth=1.5)
    ax.set_xlabel('Frequency (Hz)')
    ax.set_ylabel('Magnitude (dB)')
    ...
    ax = axes[1, 2]
    bands = sorted(set(target_profile.band_rt60.keys()) & set(silk_profile.band_rt60.keys()),
                   key=lambda x: int(x.replace('Hz', '')))
    if bands:
        x_pos = np.arange(len(bands))
        t_vals = [target_profile.band_rt60[b] for b in bands]
        s_vals = [silk_profile.band_rt60[b] for b in bands]
        w = 0.35
        ax.bar(x_pos - w/2, t_vals, w, label='Lexicon', alpha=0.8)
        ax.bar(x_pos + w/2, s_vals, w, label='SilkVerb', alpha=0.8)
        ax.set_xticks(x_pos)
...
def cmd_report(args):
    ...
    category = args.category
    ir_dir = os.path.join(LEXICON_BASE, category)
...
def main():
    parser = argparse.ArgumentParser(
        description='Lexicon PCM 90 / SilkVerb IR Matching Tool',
    ...
    # compare
    p = subparsers.add_parser('compare', help='Compare SilkVerb against a target IR')
    p.add_argument('ir_path', help='Path to .wav IR file')
    ...
    # optimize
    p = subparsers.add_parser('optimize', help='Optimize SilkVerb params for a target IR')
    p.add_argument('ir_path', help='Path to .wav IR file')
