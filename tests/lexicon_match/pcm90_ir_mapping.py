#!/usr/bin/env python3
"""
PCM 90 IR Mapping — Maps all Lexicon PCM 90 impulse responses to SilkVerb preset entries.

Generates a list of (ir_path, preset_name, silkverb_category, silkverb_mode) tuples
...
# SilkVerb mode indices (matching FDNReverb.h enum)
MODE_PLATE = 0
MODE_ROOM = 1
MODE_HALL = 2
MODE_CHAMBER = 3
...
HALLS_MAP = {
    ...
    'Concert Hall (574)':    ('Concert Hall',     'Halls', MODE_HALL,       'Dense, classic Lexicon hall'),
...
def get_all_ir_mappings() -> list[dict]:
    ...
    for pcm90_bank, mapping in all_maps:
        ir_dir = os.path.join(LEXICON_BASE, pcm90_bank)
