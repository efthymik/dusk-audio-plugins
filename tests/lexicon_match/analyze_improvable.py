#!/usr/bin/env python3
"""
Analyze "improvable" mid-range presets from SilkVerb v2 batch optimization.

Identifies the 3 worst-scoring normal presets (score >= 40) per category,
re-renders SilkVerb IRs with their optimized params, and produces detailed
...
from silkverb_capture import SilkVerbParams, capture_ir
...
def analyze_preset(preset_result: dict) -> dict:
    ...
    1. Load target IR
    2. Render SilkVerb IR with optimized params
    ...
    # Render SilkVerb IR with optimized params
    print(f"  Rendering SilkVerb IR with optimized params...")
    params = SilkVerbParams.from_dict(params_dict)

    # Determine capture duration based on target RT60 (at least 3x RT60 or 6s)
    capture_duration = max(6.0, target_profile.rt60 * 3.0)
    silkverb_ir = capture_ir(params, sr=target_sr, duration_s=capture_duration)
    silkverb_profile = analyze_ir(silkverb_ir, target_sr, name=f"SilkVerb: {name}")

    # Compare
    comparison = compare_profiles(target_profile, silkverb_profile)
    ...
    print(f"    SilkVerb RT60: {silkverb_profile.rt60:.3f}s")
    ...
    return {
        ...
        "silkverb_profile": silkverb_profile,
        "params": params,
    }
